...
Partial Class Reports_FrontDesk_StoreZReport
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        If Not (IsPostBack) Then
            ...
            dr = cm.ExecuteReader
            Do While dr.Read()
                If IsDBNull(dr("Closed")) Then
                    sStatus = "Open"
                Else
                    sStatus = IIf(dr("Closed"), "Closed", "Open")
                End If
                ddBatch.Items.Add(New ListItem(dr("BatchID") & " - " & dr("StartDate") & " - " & sStatus, dr("BatchID")))
            Loop
            dr.Close()
            cn.Close()
        End If
    ...
    Protected Sub Button1_Click(sender As Object, e As System.EventArgs) Handles Button1.Click
        ...
        cm.CommandText = "Select BatchID, Case when CLoseAmount is null then 0 else closeamount end as CloseAmount, OpenAmount, StartDate, Closed from t_Batch where batchID = " & ddBatch.SelectedValue ' & " and locationid = 1"
        da.Fill(ds, "Batch")

        If IsDBNull(ds.Tables("Batch").Rows(0).Item("Closed")) Then
            sStatus = "Open"
        Else
            sStatus = IIf(ds.Tables("Batch").Rows(0).Item("Closed"), "Closed", "Open")
        End If
        ...
        rpt = rpt & "<tr><td>Batch Status:</td><td>" & sStatus & "</td></tr>" 'format_Line("Batch Status", IIf(ds.Tables("Batch").Rows(0).Item("Closed"), "Closed", "Open"), lineLength) & vbCrLf '"Batch Status" & vbTab & vbTab & IIf(ds.Tables("Batch").Rows(0).Item("Closed"), "Closed", "Open") & vbCrLf
        ...
        cm.CommandText = "Select (CashAmt - CashRefAmt) as Total from  (Select (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'Cash' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 0) as CashAmt, (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'Cash' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 1) as CashRefAmt) xx"
        dr = cm.ExecuteReader
        dr.Read()
        ...
        cm.CommandText = "Select (CashAmt - CashRefAmt) as Total from  (Select (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where UPPER(p.PaymentMethod) = 'GIFTCARD' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 0) as CashAmt, (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where UPPER(p.PaymentMethod) = 'GIFTCARD' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 1) as CashRefAmt) xx"
        dr = cm.ExecuteReader
        dr.Read()
        cashTotal = CDbl(dr("Total"))
        ...
        cm.CommandText = "Select (CashAmt - CashRefAmt) as Total from  (Select (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'Check' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 0) as CashAmt, (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'Check' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 1) as CashRefAmt) xx"
        dr = cm.ExecuteReader
        dr.Read()
        ...
        cm.CommandText = "Select (CashAmt - CashRefAmt) as Total from  (Select (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'TravlersCheck' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 0) as CashAmt, (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'TravlersCheck' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 1) as CashRefAmt) xx"
        dr = cm.ExecuteReader
        dr.Read()
        ...
        cm.CommandText = "Select (CashAmt - CashRefAmt) as Total from  (Select (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'American Express' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 0) as CashAmt, (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'American Express' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 1) as CashRefAmt) xx"
        dr = cm.ExecuteReader
        dr.Read()
        ...
        cm.CommandText = "Select (CashAmt - CashRefAmt) as Total from  (Select (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'Discover' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 0) as CashAmt, (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'Discover' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 1) as CashRefAmt) xx"
        dr = cm.ExecuteReader
        dr.Read()
        ...
        cm.CommandText = "Select (CashAmt - CashRefAmt) as Total from  (Select (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'Mastercard' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 0) as CashAmt, (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'Mastercard' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 1) as CashRefAmt) xx"
        dr = cm.ExecuteReader
        dr.Read()
        ...
        cm.CommandText = "Select (CashAmt - CashRefAmt) as Total from  (Select (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'VISA' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 0) as CashAmt, (Select Case when Sum(Amount) is null then 0 else Sum(Amount) end from t_Payment p inner join t_Sales s on p.SalesID = s.SalesID where p.PaymentMethod = 'VISA' and s.BatchID = " & ddBatch.SelectedValue & " and s.RegisterID = 1 and Refunded = 1) as CashRefAmt) xx"
        dr = cm.ExecuteReader
        dr.Read()
        ...
        cm.CommandText = "Select Case when Sum(Amount) is null then 0 else Sum(Amount) end as Amt from t_CashDrop where batchID = " & ddBatch.SelectedValue & " and Payout = 0"
        dr = cm.ExecuteReader
        dr.Read()
        rpt = rpt & "<tr><td>Cash Drops:</td><td>" & FormatCurrency(dr("Amt"), 2) & "</td></tr>"
        cashTotal = cashTotal + dr("Amt")
        dr.Close()

        cm.CommandText = "Select Case when Sum(Amount) is null then 0 else Sum(Amount) end as Amt from t_CashDrop where batchID = " & ddBatch.SelectedValue & " and Payout = 1"
        dr = cm.ExecuteReader
        dr.Read()
        rpt = rpt & "<tr><td>Cash Payouts:</td><td>" & FormatCurrency(dr("Amt"), 2) & "</td></tr>"
        cashTotal = cashTotal + dr("Amt")
        dr.Close()
