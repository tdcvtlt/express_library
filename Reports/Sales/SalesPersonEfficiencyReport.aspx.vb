...
Partial Class Reports_Sales_SalesPersonEfficiencyReport
    ...
    Protected Sub btRunReport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btRunReport.Click
        ...
        sql(0) = String.Format( _
            ...
            "inner join t_PersonnelTrans pt on pt.personnelid = p.personnelid inner join t_Comboitems ps on p.StatusID = ps.ComboItemID " & _
            "where ps.ComboItem = 'Active' and (pt.datecreated >  '{0}' and pt.datecreated < '{1}' " & _
            ...
            "between '{0}' and '{1}' and c.statusid in  " & _
            "(Select Comboitemid from t_ComboItems A INNER JOIN T_COMBOS B ON " & _
            "A.COMBOID = B.COMBOID where ComboName = 'ContractStatus' and comboitem in ('Active','Suspense','DEVELOPER') " & _
        ...
        sql(1) = String.Format( _
            "Select p.personnelID, Count(Distinct pt.KEYVALUE) as Tours from t_Personnel p inner join t_Comboitems ps on p.StatusID = ps.ComboItemID inner join t_PersonnelTrans pt on pt.personnelid = p.personnelid  " & _
            "inner join t_Tour t on t.tourid = pt.KEYVALUE AND PT.KEYFIELD = 'TOURID' " & _
            "where ps.ComboItem = 'Active' and pt.datecreated > '{0}' and pt.datecreated < '{1}' and  pt.KEYVALUE > 0  AND PT.KEYFIELD = 'TOURID' " & _
        ...
        sql(2) = String.Format( _
            "Select p.personnelID, Count(Distinct pt.KEYVALUE) as Contracts from t_Personnel p inner join t_Comboitems ps on p.StatusID = ps.ComboItemID inner join t_PersonnelTrans pt on pt.personnelid = p.personnelid  " & _
            "inner join t_Contract c on c.contractid = pt.KEYVALUE AND PT.KEYFIELD = 'CONTRACTID'  " & _
            "where ps.ComboItem = 'Active' and ((c.contractdate < '{0}' and c.statusdate between '{0}' and '{1}' and c.contractid in " & _
         ...
         "(c.contractdate between '{0}' and  '{1}')) and  pt.KEYVALUE > 0  " & _
            ...
            "where ComboName = 'ContractStatus' and comboitem in ('Active','Suspense','DEVELOPER')) and c.contractid not in  " & _
        ...
        sql(3) = String.Format( _
            "Select p.personnelID, Count(Distinct pt.KEYVALUE) as Contracts from t_Personnel p inner join t_Comboitems ps on p.StatusID = ps.ComboItemID inner join t_PersonnelTrans pt on pt.personnelid = p.personnelid  " & _
            "inner join t_Contract c on c.contractid = pt.KEYVALUE AND PT.KEYFIELD = 'CONTRACTID'  " & _
            "where ps.ComboItem = 'Active' and (c.contractdate between '{0}' and '{1}') and  PT.KEYFIELD = 'CONTRACTID' AND  pt.KEYVALUE > 0  " & _
            "and pt.titleid in  ({2})  and c.statusid in  " & _
            "(Select Comboitemid from t_ComboItems A INNER JOIN T_COMBOS B ON A.COMBOID = B.COMBOID where ComboName = 'ContractStatus' and comboitem in ('Active','Suspense','DEVELOPER')) " & _
        ...
        sql(4) = String.Format( _
            "Select p.personnelID, Count(Distinct pt.KEYVALUE) as Penders from t_Personnel p inner join t_Comboitems ps on p.StatusID = ps.ComboItemID inner join t_PersonnelTrans pt on pt.personnelid = p.personnelid  " & _
            "inner join t_Contract c on c.contractid = pt.KEYVALUE AND PT.KEYFIELD = 'CONTRACTID' " & _
            "where ps.ComboItem = 'Active' and c.contractdate between '{0}' and '{1}' and  pt.KEYVALUE > 0  " & _
            "and pt.titleid in  ({2})  and c.statusid in  " & _
        ...
        sql(5) = String.Format( _
            ...
            "(Select Comboitemid from t_ComboItems A INNER JOIN T_COMBOS B ON A.COMBOID = B.COMBOID " & _
            "where ComboName = 'ContractStatus' and comboitem in ('Active','Suspense','DEVELOPER')) and t.personnelid = p.personnelid and  " & _
            ...
            "from t_Personnel p inner join t_Comboitems ps on p.StatusID = ps.ComboItemID inner join t_PersonnelTrans pt on pt.personnelid = p.personnelid  " & _
            "          inner join t_Contract c on c.contractid = pt.KEYVALUE AND PT.KEYFIELD = 'CONTRACTID'  " & _
            "inner join t_Mortgage m on m.contractid = c.contractid  " & _
            "          where ps.ComboItem = 'Active' and ((pt.datecreated >= '{0}' " & _
            ...
            "and comboitem in ('Active','Suspense','DEVELOPER'))  and c.contractid not in  " & _
            "(Select Contractid from t_Contract where contractnumber like 't%' and contractnumber not like 'T%')  group by p.personnelid", _
            start, finish, title)
        ...
        sql(6) = String.Format( _
            ...
            "ON A.COMBOID = B.COMBOID where ComboName = 'ContractStatus' and comboitem in ('Active','Suspense','DEVELOPER'))  " & _
            ...
            "from t_Personnel p inner join t_Comboitems ps on p.StatusID = ps.ComboItemID inner join " & _
            "t_PersonnelTrans pt on pt.personnelid = p.personnelid  " & _
            "          inner join t_Contract c on c.contractid = pt.KEYVALUE AND PT.KEYFIELD = 'CONTRACTID'  " & _
            "inner join t_Mortgage m on m.contractid = c.contractid  " & _
            "          where ps.ComboItem = 'Active' and ((pt.datecreated >= '{0}' and " & _
        ...
        sql(7) = String.Format( _
            ...
            "from t_Personnel p inner join t_Comboitems ps on p.StatusID = ps.ComboItemID inner join t_PersonnelTrans pt on " & _
            "        pt.personnelid = p.personnelid " & _
            "          inner join t_Contract c on c.contractid = pt.KEYVALUE AND PT.KEYFIELD = 'CONTRACTID'  " & _
            "inner join t_Mortgage m on m.contractid = c.contractid  " & _
            "          where ps.ComboItem = 'Active' and c.contractdate between  '{0}' and '{1}' and  pt.KEYVALUE > 0 AND PT.KEYFIELD = 'CONTRACTID'  " & _
            "          and pt.titleid in  " & _
            " ({2})  and c.statusid in  " & _
        ...
        sql(8) = String.Format( _
            "Select p.personnelID, Count(Distinct pt.KEYVALUE) as Penders from t_Personnel p inner join t_Comboitems ps on p.StatusID = ps.ComboItemID inner join t_PersonnelTrans pt on pt.personnelid = p.personnelid  " & _
            "inner join t_Contract c on c.contractid = pt.KEYVALUE AND PT.KEYFIELD = 'CONTRACTID' " & _
            "where ps.ComboItem = 'Active' and c.contractdate between '{0}' and '{1}' and  pt.KEYVALUE > 0  " & _
            "and pt.titleid in  ({2})  and c.statusid in  " & _
            "(Select Comboitemid from t_ComboItems A INNER JOIN T_COMBOS B ON A.COMBOID = B.COMBOID where ComboName = 'ContractStatus' and comboitem in " & _
            "('Rescinded'))  and c.contractid not in  " & _
            "(select contractid from t_Event E where E.KEYVALUE > 0 AND E.KEYFIELD = 'CONTRACTID' and " & _
            "(oldvalue = 'pender' or oldvalue = 'Pender-Inv'))   group by p.personnelid", _
            start, finish, title)

        sql(9) = String.Format( _
            "Select p.personnelID, (Select Sum(salesvolume) from t_Mortgage where " & _
            "mortgageid in (select distinct mortgageid from t_contract c inner join " & _
            "t_Personneltrans t on t.KEYVALUE = c.contractid AND T.KEYFIELD = 'CONTRACTID' inner join " & _
            "t_Mortgage m on m.contractid = c.contractid where c.statusdate " & _
                        ...
                        While rdr.Read()
                            ...
                            list.Add(rdr.Item(0), New Efficiency() _
                                     With {.Name = rdr.Item(1) & ", " & rdr.Item(2), _
                                            .Contracts = New Integer() {0, 0, 0}, .Volume = New Integer() {0, 0, 0, 0}})
                        End While
                        rdr.Close()
        ...
        html.Append("<tr><td>Personnel</td><td>Tours</td><td>Active Contracts</td><td>OW Contracts</td><td>Pending</td><td>AC Volume</td><td>VPG</td><td>Pending CV</td><td>Rescinded</td><td>Rescinded Vol</td></tr>")
        ...
        For Each ele As KeyValuePair(Of Integer, Efficiency) In list
            ...
            html.AppendFormat("<td>{0}</td>", ncy.Contracts(2))
            html.AppendFormat("<td>{0:C2}</td>", ncy.Volume(3))
    ...
    Private Function LoopRows(ByRef index As Integer, ByRef rdr As SqlDataReader, _
        ...
        Do While rdr.Read()
            ...
            If IsDBNull(rdr.Item(1)) = False Then
                ...
                If index = 8 Then e.Contracts(2) = rdr.Item(1)
                If index = 9 Then e.Volume(3) = rdr.Item(1)
            End If
        Loop
