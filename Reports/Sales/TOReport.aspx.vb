...
Partial Class Reports_Sales_TOreport
    ...
    Protected Sub btRunReport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btRunReport.Click
                        ...
                        While rdr.Read()
                            list.Add(rdr.Item(0), New Efficiency() _
                                     With {.Name = rdr.Item(1) & ", " & rdr.Item(2), _
                                            .Contracts = New Integer() {0, 0}, _
                                           .Volume = New Integer() {0, 0, 0}})
                        End While
                        rdr.Close()
                    Else
        ...
        html.Append("<tr><td>Personnel</td><td>Tours</td><td>Active Contracts</td>" & _
                    "<td>Active Contract Volumne</td><td>Closing Percentage</td>" & _
                    "<td>VPG</td><td>Pending Contracts</td><td>Pending Contract Volume</td></tr>")
        ...
        For Each ele As KeyValuePair(Of Integer, Efficiency) In list
            ...
            html.AppendFormat("<tr><td>{0}</td>", ncy.Name)
            html.AppendFormat("<td>{0}</td>", ncy.Tours)
            html.AppendFormat("<td>{0}</td>", ncy.Contracts(0))
            html.AppendFormat("<td>{0:C2}</td>", ncy.Volume(0))
            html.AppendFormat("<td>{0}</td>", FormatPercent((ncy.Contracts(0) / ncy.Tours), 2))
            html.AppendFormat("<td>{0:C2}</td>", ncy.Volume(0) / ncy.Tours)
            html.AppendFormat("<td>{0}</td>", ncy.Pender)
            html.AppendFormat("<td>{0:C2}</td></tr>", ncy.Volume(2))
        Next

        If list.Count > 0 Then
            html.Append(String.Format( _
                "<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3:C2}</td><td>{4}</td>" & _
                "<td>{5:C2}</td><td>{6}</td><td>{7:C2}</td></tr>", _
                "Total", list.Sum(Function(x) x.Value.Tours), _
                        list.Sum(Function(x) x.Value.Contracts(0)), _
                        list.Sum(Function(x) x.Value.Volume(0)), _
                        FormatPercent(list.Sum(Function(x) x.Value.Contracts(0)) / list.Sum(Function(x) x.Value.Tours), 2), _
                        list.Sum(Function(x) x.Value.Volume(0)) / list.Sum(Function(x) x.Value.Tours), _
    ...
    Private Sub WhatsWrong(ByVal rc As SqlDataReader)

        While rc.Read
            Response.Write(rc.Item(0) & "<br/>")
        End While
