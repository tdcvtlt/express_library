...
<BODY>
<font size = '5'>Exit Daily Trans Report</font>
<br>
<%
...
DBPass = "aspnet"
	...
	Else
		...
		Do while not rst.EOF
			...
			rst2.Open "Select e.*, f.Batch from (Select c.*, d.ComboItem as ccType from (Select a.Number, a.TypeID, b.batchid from t_CreditCard a inner join t_CCTrans b on a.creditcardid = b.creditcardid where b.Applyto Like '%" & rst.Fields("AccountItemID") & "%') c left outer join t_ComboItems d on c.typeid = d.comboitemid) e left outer join t_CCBatch f on e.batchid = f.batchid", cn, 3, 3
			...
			elseif IsNull(rst2.Fields("Number")) or rst2.Fields("Number") = "" then
				...
				response.write "<td>" & rst2.Fields("batch") & "</td>"
	...
	grandamt = amt
	amt = 0
	rst.Open "Select g.*, h.COmboItem as pMethod from (Select e.*, f.FirstName as FirstName, f.LastName as LastName from (Select c.*, d.ContractNumber from (Select a.*, b.ContractID as conID from (Select * from t_AccountItems where transactiondate between '" & request("sDate") & "' and '" & request("eDate") & "' and paid = '1') a inner join t_Mortgage b on a.mortgageid = b.mortgageid) c inner join t_Contract d on c.conid = d.contractid) e inner join t_prospect f on e.prospectid = f.prospectid) g left outer join t_ComboItems h on g.paymentmethodid = h.comboitemid where (ContractNumber LIKE 'T%' or ContractNumber like 'U%') and comboitem not in (" & creditcards & ") or h.comboitem is null order by ComboItem asc", cn, 3, 3
	If rst.EOF and rst.BOF then
		response.write "<table>"
