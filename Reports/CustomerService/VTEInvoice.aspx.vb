...
Partial Class Reports_CustomerService_VTEInvoice
    ...
    Protected Sub Unnamed1_Click(sender As Object, e As System.EventArgs)
        ...
        sANs = "<H2>VTE Invoice " & sdate & " - " & edate & "</H2>"
        sANs = sANs & "<table>"
        sANs = sANs & "<tr>"
    ...
    Private Sub Old_Code()
        ...
        Else
            Do While Not rs.EOF
                ...
                Else
                    ...
                    Else
                        ...
                        Else
                            rs2.Close()
                            rs2.Open("Select Sum(p.Amount) as CBAmount from t_Invoices i inner join t_Payment2Invoice pi on i.InvoiceID = pi.InvoiceID inner join t_Payments p on pi.PaymentID = p.PaymentID inner join t_COmboItems pm on p.MethodID = pm.ComboItemID where i.KeyField = 'ReservationID' and i.KeyValue = " & rs.Fields("ReservationID").value & " and pm.ComboItem = 'Chargeback - VTE'", cn, 3, 3)
                            'rs2.Open("Select Sum(Amount) as CBAmount from t_AccountItems where paymentmethodid = (Select comboitemid from t_ComboItems where comboname = 'PaymentMethod' and comboitem = 'ChargeBack - VTE') and reservationid = '" & rs.FIelds("ReservationID") & "'", cn, 3, 3)
                            If (rs2.EOF And rs2.BOF) Or IsDBNull(rs2.Fields("CBAmount").Value) Then
                                AccomCost = 0
                            Else
                                AccomCost = rs2.Fields("CBAmount").Value
                ...
                sANs = sANs & "<td>" & FormatCurrency(premCost) & "</td>"
                If rs.Fields("TourStatus").Value = "No Tour - Overage" Or rs.Fields("TourStatus").Value = "Showed" Or rs.Fields("TourStatus").Value = "OnTour" Or (rs.Fields("TourStatus").Value = "NQ - Toured" And Not (IsDBNull(rs.Fields("ContractNumber").Value))) Then
                    'If Month(rs.Fields("CheckIndate").Value) < 5 Or Month(rs.Fields("CheckInDate").Value) > 9 Or (Month(rs.Fields("CheckInDate").Value) = 5 And Day(rs.Fields("CheckIndate").Value) < 27) Or (Month(rs.Fields("CheckInDate").Value) = 9 And Day(rs.Fields("CheckInDate").Value) > 6) Then
                    'tourFee = 125
                    'If premCost > 60 Then
                    'premCost = premCost - 60
                    'Else
                    'premCost = 0
                    'End If
                    'If twoBD = True Then
                    'tourCost = tourFee - premCost - 50
                    'Else
    ...
    Protected Sub Button3_Click(sender As Object, e As System.EventArgs) Handles Button3.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=VTEInvoice.xls")
