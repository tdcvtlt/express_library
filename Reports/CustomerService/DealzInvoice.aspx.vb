...
Partial Class Reports_CustomerService_DealzInvoice
    ...
    Private Sub Old_Code()
        ...
        sANs = "<H2>Dealz Invoice " & sdate & " - " & edate & "</H2>"
        sANs = sANs & "<table>"
        sANs = sANs & "<tr>"
                ...
                If CInt(rs.Fields("TourID").Value) <> prevID Then
                    ...
                    Else
                        ...
                        Else
                            ...
                            Else
                                rs2.Close()
                                rs2.Open("Select Sum(p.Amount) as CBAmount from t_Invoices i inner join t_Payment2Invoice pi on i.InvoiceID = pi.InvoiceID inner join t_Payments p on pi.PaymentID = p.PaymentID inner join t_COmboItems pm on p.MethodID = pm.ComboItemID where i.KeyField = 'ReservationID' and i.KeyValue = " & rs.Fields("ReservationID").Value & " and pm.ComboItem = 'Chargeback - Dealz'", cn, 3, 3)
                                'rs2.Open("Select Sum(Amount) as CBAmount from t_AccountItems where paymentmethodid = (Select comboitemid from t_ComboItems where comboname = 'PaymentMethod' and comboitem = 'ChargeBack - Dealz') and reservationid = '" & rs.FIelds("ReservationID") & "'", cn, 3, 3)
    ...
    Protected Sub Button3_Click(sender As Object, e As System.EventArgs) Handles Button3.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=DealzInvoice.xls")
