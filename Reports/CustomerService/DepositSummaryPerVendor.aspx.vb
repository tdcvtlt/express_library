...
Partial Class Reports_CustomerService_DepositSummaryPerVendor
    Inherits System.Web.UI.Page

    Dim Report As New ReportDocument
    Dim sReport As String = "reportfiles/DepositSummaryPerVendor.rpt"
            ...
            Using cn = New SqlConnection(Resources.Resource.cns)
                Using cm = New SqlCommand()
                    cm.CommandText = String.Format("SELECT Distinct(f.FinTransID) AS ID, tc.ComboItem AS Item " & _
                        ...
                        "WHERE vp.VendorID = {0};select c.ComboItemID, c.ComboItem from t_ComboItems c inner join t_Combos co " & _
                        "on c.ComboID = co.COmboID where co.comboname = 'PaymentMethod' and c.active = '1' order by comboitem asc", CType(Session("user"), User).ActiveVendor)
                    cm.Connection = cn
                    ...
                    Try
                        ...
                        ddlPaymentMethod.AppendDataBoundItems = True
                        ddlPaymentMethod.Items.Add("All")

                        ddlPaymentMethod.DataValueField = "ComboItemID"
                        ddlPaymentMethod.DataTextField = "ComboItem"
                        ddlPaymentMethod.DataSource = rd
    ...
    Protected Sub submit_Click(sender As Object, e As System.EventArgs) Handles submit.Click
        ...
        If ddlPaymentMethod.SelectedItem.Text = "All" Then
            Report.SetParameterValue("methodID", String.Join(",", ddlPaymentMethod.Items.OfType(Of ListItem).Where(Function(x) x.Text.Equals("All") = False).Select(Function(x) String.Format("{0}", x.Value)).ToArray()))
        Else
            Report.SetParameterValue("methodID", ddlPaymentMethod.SelectedItem.Value)
        End If
