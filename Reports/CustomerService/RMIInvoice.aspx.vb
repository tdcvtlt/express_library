...
Partial Class Reports_CustomerService_RMIInvoice
    ...
    Protected Sub Unnamed1_Click(sender As Object, e As System.EventArgs) Handles Button1.Click
        ...
        sANs = "<H2>RMI Invoice " & sdate & " - " & edate & "</H2>"
        sANs = sANs & "<table>"
        sANs = sANs & "<tr>"
        ...
        Else
            Do While Not rs.EOF
                ...
                If rs.Fields("ReservationLocation").Value & "" = "Williamsburg" Then
                    ...
                    'rs2.OPen("Select c.*, d.ComboItem as RoomType from (Select b.ComboItem As Accommodation, " & _
                    '         "a.RoomTypeID, a.ArrivalDate, a.DepartureDate, DateDiff(d, a.ArrivalDate, a.DepartureDate) As Nights, " & _
                    '         "Month(a.ArrivalDate) As Month from t_Accommodation a inner join t_ComboItems b on a.LodgingID = b.comboitemid " & _
                    '         "where a.reservationid = '" & rs.Fields("ReservationID").Value & "') c left outer join " & _
                    '         "t_ComboItems d on c.RoomTypeID = d.ComboItemID", cn, 3, 3)


                    rs2.Open(String.Format("select b.AccomName [Accommodation], a.ArrivalDate, a.DepartureDate, DateDiff(d, a.ArrivalDate, " & _
                                  "a.DepartureDate) As Nights from t_Accommodation a inner join t_Accom  b " & _
                                  "on a.AccomID = b.AccomID where a.ReservationID = {0}", rs.Fields("ReservationID").Value), cn, 3, 3)
                ...
                Else
                    ...
                    Else
                        ...
                        Else
                            rs2.Close()
                            rs2.Open("Select Sum(p.Amount) as CBAmount from t_Invoices i inner join t_Payment2Invoice pi on i.InvoiceID = pi.InvoiceID inner join t_Payments p on pi.PaymentID = p.PaymentID inner join t_COmboItems pm on p.MethodID = pm.ComboItemID where i.KeyField = 'ReservationID' and i.KeyValue = " & rs.Fields("ReservationID").value & " and pm.ComboItem = 'Chargeback - RMI'", cn, 3, 3)
                            'rs2.Open("Select Sum(Amount) as CBAmount from t_AccountItems where paymentmethodid = (Select comboitemid from t_ComboItems where comboname = 'PaymentMethod' and comboitem = 'ChargeBack - RMI') and reservationid = '" & rs.FIelds("ReservationID") & "'", cn, 3, 3)
    ...
    Protected Sub Button3_Click(sender As Object, e As System.EventArgs) Handles Button3.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=RMIInvoice.xls")
