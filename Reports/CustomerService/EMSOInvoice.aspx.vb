...
Partial Class Reports_CustomerService_EMSOInvoice
    ...
    Protected Sub Unnamed1_Click(sender As Object, e As System.EventArgs)
        ...
        sANs = "<H2>EMSO Invoice " & sdate & " - " & edate & "</H2>"
        sANs = sANs & "<table>"
        sANs = sANs & "<tr>"
            ...
            Do While Not rs.EOF
                ...
                Else
                    ...
                    Else
                        ...
                        Else
                            rs2.Close()
                            rs2.Open("Select Sum(p.Amount) as CBAmount from t_Invoices i inner join t_Payment2Invoice pi on i.InvoiceID = pi.InvoiceID inner join t_Payments p on pi.PaymentID = p.PaymentID inner join t_COmboItems pm on p.MethodID = pm.ComboItemID where i.KeyField = 'ReservationID' and i.KeyValue = " & rs.Fields("ReservationID").Value & " and pm.ComboItem = 'Chargeback - EMSO'", cn, 3, 3)
                            'rs2.Open("Select Sum(Amount) as CBAmount from t_AccountItems where paymentmethodid = (Select comboitemid from t_ComboItems where comboname = 'PaymentMethod' and comboitem = 'ChargeBack - EMSO') and reservationid = '" & rs.FIelds("ReservationID") & "'", cn, 3, 3)
