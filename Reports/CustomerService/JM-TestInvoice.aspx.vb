...
Partial Class Reports_CustomerService_JM_TestInvoice
    ...
    Protected Sub Unnamed1_Click(sender As Object, e As System.EventArgs)
        ...
        Dim check50 As Boolean = True
        Dim unitType As String = ""
        sANs = "<H2>J&M-Test Invoice " & sdate & " - " & edate & "</H2>"
        ...
        sANs = sANs & "<th><u>Contract</u></th>"
        sANs = sANs & "<th><u>Contract Date</u></th>"
        sANs = sANs & "<th><u>Contract Status</u></th>"
        ...
        cn.CommandTimeout = 0
        rs.Open("Select m.*, n.ComboItem as ContractStatus from (SELECT k.*, l.FirstName AS Firstname, l.LastName AS Lastname FROM (SELECT i.*, j.ContractDate as ContractDate, j.ContractNumber as COntractNumber, j.StatusID as ContractStatusID FROM (SELECT g.*, h.ComboItem AS ReservationLocation FROM (SELECT e.*, f.ComboItem AS ResStatus FROM (SELECT c.*, d .ReservationID AS ReservationID, d .CheckInDate AS CheckInDate, d.DateBooked as DateBooked, d .CheckOutDate AS CheckOutDate, d .StatusID AS StatusID, d .ResLocationID AS ResLocationID, DateDiff(d, CheckInDate, CheckOutDate) As ResNights FROM (SELECT a.TourDate, a.TourID, a.ProspectID, b.ComboItem AS TourStatus FROM t_Tour a LEFT OUTER JOIN t_ComboItems b ON a.StatusID = b.ComboItemID WHERE (a.CampaignID = (SELECT campaignid FROM t_Campaign WHERE name = 'J&M-Test') AND a.tourdate between '" & sdate & "' and '" & edate & "' AND a.SubTypeID NOT IN (SELECT c.comboitemid FROM t_ComboItems c inner join t_Combos co on c.ComboID = co.ComboID WHERE co.comboname = 'TourSubType' AND c.comboitem = 'Exit'))) c LEFT OUTER JOIN t_Reservations d ON c.TourID = d .TourID) e LEFT OUTER JOIN t_ComboItems f ON e.StatusID = f.ComboItemID) g LEFT OUTER JOIN t_ComboItems h ON g.ResLocationID = h.ComboItemID) i LEFT OUTER JOIN t_Contract j ON i.TourID = j.TourID) k LEFT OUTER JOIN t_Prospect l ON k.ProspectID = l.ProspectID) m left outer join t_ComboItems n on m.ContractStatusID = n.ComboItemID order by tourdate asc", cn, 3, 3)
            ...
            Do While Not rs.EOF
                ...
                Else
                    ...
                    Else
                        ...
                        Else
                            rs2.Close()
                            rs2.Open("Select Sum(p.Amount) as CBAmount from t_Invoices i inner join t_Payment2Invoice pi on i.InvoiceID = pi.InvoiceID inner join t_Payments p on pi.PaymentID = p.PaymentID inner join t_COmboItems pm on p.MethodID = pm.ComboItemID where i.KeyField = 'ReservationID' and i.KeyValue = " & rs.Fields("ReservationID").value & " and pm.ComboItem = 'Chargeback - JandM'", cn, 3, 3)
                            'rs2.Open("Select Sum(Amount) as CBAmount from t_AccountItems where paymentmethodid = (Select comboitemid from t_ComboItems where comboname = 'PaymentMethod' and comboitem = 'ChargeBack - J&M') and reservationid = '" & rs.FIelds("ReservationID") & "'", cn, 3, 3)
                ...
                Else
                    counter = 0
                    Do While Not rs2.EOF
                        If rs2.Fields("PremiumName").Value = "Check" And rs2.Fields("TotalCost").Value = 50 Then
                            If check50 = False Then
                                check50 = True
                            Else
                                If counter = 0 Then
                ...
                If rs.Fields("ContractNumber").Value & "" <> "" And (rs.Fields("ContractStatus").Value = "Active" Or rs.Fields("ContractStatus").Value = "Suspense" Or rs.Fields("ContractStatus") = "Developer" Or rs.Fields("ContractStatus").Value = "Pender" Or rs.Fields("ContractStatus").value = "Pender-INV") Then
                    tourFee = 300
                Else
                    tourFee = 0
                End If
    ...
    Protected Sub Button3_Click(sender As Object, e As System.EventArgs) Handles Button3.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=JM-TestInvoice.xls")
