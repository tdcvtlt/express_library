...
Partial Class Reports_CustomerService_PVGInvoice
    ...
    Private Sub Old_Code()
        ...
        sANs = "<H2>AMG Invoice " & sdate & " - " & edate & "</H2>"
        sANs = sANs & "<table>"
        sANs = sANs & "<tr>"
                ...
                If CInt(rs.Fields("TourID").Value) <> prevID Then
                    ...
                    Else
                        ...
                        Else
                            ...
                            Else
                                rs2.Close()
                                rs2.Open("Select Sum(p.Amount) as CBAmount from t_Invoices i inner join t_Payment2Invoice pi on i.InvoiceID = pi.InvoiceID inner join t_Payments p on pi.PaymentID = p.PaymentID inner join t_COmboItems pm on p.MethodID = pm.ComboItemID where i.KeyField = 'ReservationID' and i.KeyValue = " & rs.Fields("ReservationID").Value & " and pm.ComboItem = 'Chargeback - AMG'", cn, 3, 3)
                                'rs2.Open("Select Sum(Amount) as CBAmount from t_AccountItems where paymentmethodid = (Select comboitemid from t_ComboItems where comboname = 'PaymentMethod' and comboitem = 'ChargeBack - AMG') and reservationid = '" & rs.FIelds("ReservationID") & "'", cn, 3, 3)
                    ...
                    If rs.Fields("TourStatus").Value = "Showed" Or rs.Fields("TourStatus").Value = "OnTour" Or rs.Fields("TourStatus").Value = "No Tour - Overage" Or (rs.Fields("TourStatus").Value = "NQ - Toured" And Not (IsDBNull(rs.Fields("ContractNumber").Value))) Then
                        tourFee = 375
                    Else
                        tourFee = 0
                    End If
    ...
    Protected Sub Unnamed1_Click(sender As Object, e As System.EventArgs)
        ...
        Dim grandTotal As Double = 0
        Dim unitType As String = ""
        sANs = "<H2>PVG Invoice " & sdate & " - " & edate & "</H2>"
        sANs = sANs & "<table>"
        sANs = sANs & "<tr>"
        sANs = sANs & "<th><u>TourID</u></th>"
        sANs = sANs & "<th><u>TourDate</u></th>"
    ...
    Protected Sub Button3_Click(sender As Object, e As System.EventArgs) Handles Button3.Click
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=AMGInvoice.xls")
