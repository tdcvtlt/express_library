...
Partial Class marketing_EditReservation
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not (IsPostBack) Then
            ...
            If CheckSecurity("Reservations", "View", , , Session("UserDBID")) Then
                ...
                If IsNumeric(Request("ReservationID")) Then
                    If CInt(Request("ReservationID")) > 0 Then
                        ...
                        If Not (oE.Find_View_Event("ReservationID", Request("ReservationID"), Resources.Resource.ViewEventTime, Session("UserDBID"), sErr)) Then
                            If sErr <> "" Then Response.Write(sErr)
                            If Not (oE.Create_View_Event("ReservationID", Request("ReservationID"), Resources.Resource.ViewEventTime, Session("UserDBID"), sErr)) Then
                                If sErr <> "" Then Response.Write(sErr)
                            End If
                        End If
                        oE = Nothing
        ...
        Logic_Print_Or_Email_Owner_Ltr()
        Logic_Print_Or_Email_Rental_Ltr()
    ...
    Private Sub Set_Values()
        ...
        End If
        If Request("ReservationID") <> "0" Then
            If Not Request("ReservationID") Is Nothing Then
                List_Usages()
            End If
        Else
        ...
        If txtReservationID.Text > 0 Then
            ...
            If oCombo.Lookup_ComboItem(siStatus.Selected_ID) = "Booked" And oCombo.Lookup_ComboItem(oRes.StatusID) <> "Booked" And (oCombo.Lookup_ComboItem(siResType.Selected_ID) = "Marketing" Or oTour.Get_Tour_By_Res(oRes.ReservationID) > 0) Then
                ...
                If oDNS.Get_Status(oRes.ProspectID) = "Remove" Then
                    ClientScript.RegisterClientScriptBlock(Me.GetType, "pop", "modal.mwindow.open('DoNotSellListOverRide.aspx?KeyField=Reservation&KeyValue=" & txtReservationID.Text & "&Source=BookReservation','win01',350,350);", True)
                Else
                    Save_Res()
                End If
                oDNS = Nothing
            ...
            End If
        Else
            If oCombo.Lookup_ComboItem(siStatus.Selected_ID) = "Booked" And oCombo.Lookup_ComboItem(siStatus.Selected_ID) = "Marketing" Then
                Dim oDNS As New clsDoNotSellList
                If oDNS.Get_Status(oRes.ProspectID) = "Remove" Then
                    ClientScript.RegisterClientScriptBlock(Me.GetType, "pop", "modal.mwindow.open('DoNotSellListOverRide.aspx?KeyField=Reservation&KeyValue=" & txtReservationID.Text & "&Source=BookReservation','win01',350,350);", True)
                Else
    ...
    Private Sub Save_Res()
        Dim oRes As New clsReservations
        Dim bProceed As Boolean = True
        Dim err As String = ""
        If txtReservationID.Text > 0 Then
            If CheckSecurity("Reservations", "Edit", , , Session("UserDBID")) Then
                oRes.ReservationID = txtReservationID.Text
                oRes.Load()
                If oRes.CheckInDate & "" <> "" And oRes.CheckOutDate & "" <> "" Then
                    If txtCheckInDate.Text = "" Or txtCheckOutDate.Text = "" Then
                ...
                If oRes.LockInventory And Not (ckLockedInventory.Checked) Then
                    If Not (CheckSecurity("Reservations", "UnLockInventory", , , Session("UserDBID"))) Then
                        bProceed = False
            ...
            End If
        Else
            If Not (CheckSecurity("Reservations", "Add", , , Session("UserDBID"))) Then
                bProceed = False
    ...
    Private Sub Update_Values()
        ...
        Dim date_checkin = oRes.CheckInDate

        If date_checkin.Length > 0 And oRes.CheckInDate.ToString().Length > 0 Then
            If DateTime.Compare(DateTime.Parse(oRes.CheckInDate), DateTime.Parse(date_checkin)) > 0 Then
                Using cn = New SqlConnection(Resources.Resource.cns)
                    Using ad = New SqlDataAdapter("select comboitemid from t_ComboItems ci inner join t_Combos c on ci.ComboID = c.ComboID " &
                                                  "where c.ComboName	= 'reservationconfirmationstatus' and ci.Active=1 and ci.comboitem = 'confirmed'", cn)
                        ...
                        Try
                            ...
                            If dt.Rows.Count = 1 Then
                                ad.SelectCommand.CommandText = "select confirmationstatusid from t_reservations where reservationid = " & oRes.ReservationID
                                If cn.State = ConnectionState.Closed Then cn.Open()

                                Dim r = ad.SelectCommand.ExecuteScalar()
                                If IsDBNull(r) = False Then
                                    If Int32.Parse(dt.Rows(0)(0).ToString()) = CType(r, Int32) Then


                                        ad.SelectCommand.CommandText = String.Format("update t_reservations set confirmationstatusid = null where reservationid = {0}" &
                                                                                     ";insert into t_note(keyfield, keyvalue, note, datecreated, createdbyid) values " &
                                                                                     "('reservationconfirmationstatusid', {0}, 'Check-in Date has changed since last confirmation', getdate(), {1} );",
                                                                                     oRes.ReservationID, Session("UserDBID"))
    ...
    Protected Sub CIButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles CIButton.Click
        If txtReservationID.Text > 0 Then
            ...
            Else
                If CheckSecurity("Reservations", "CheckIn", , , Session("UserDBID")) Then
    ...
    Protected Sub gvRoomList1_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles gvRoomList1.RowCommand
        ...
        If e.CommandName.CompareTo("RemoveRoom") = 0 Then
            ...
            oRes.Load()
            oRes.UserID = Session("UserDBID")
            If CheckSecurity("Reservations", "Remove" & oCombo.Lookup_ComboItem(oRes.TypeID) & "Room", , , Session("UserDBID")) Then
    ...
    Protected Sub COButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles COButton.Click
        If CheckSecurity("Reservations", "CheckOut", , , Session("UserDBID")) Then
            ClientScript.RegisterClientScriptBlock(Me.GetType, "AjaxCall", "modal.mwindow.open('" & Request.ApplicationPath & "/marketing/confirmResCO.aspx?ReservationID=" & txtReservationID.Text & "','win01',250,275);", True)
        Else
    ...
    Protected Sub btnPrintLtr_Click(sender As Object, e As System.EventArgs) Handles btnPrintLtr.Click
        ...
        If CheckSecurity("Owners", "Print Letter", , , Session("UserDBID")) Or CheckSecurity("Owners", "AlwaysShowLetter",,, Session("UserDBID")) Then
    ...
    Protected Sub btnEmailLtr_Click(sender As Object, e As System.EventArgs) Handles btnEmailLtr.Click
        ...
        If CheckSecurity("Owners", "Email Letter", , , Session("UserDBID")) Or CheckSecurity("Owners", "AlwaysShowLetter",,, Session("UserDBID")) Then
            With New clsReservations
                If .Get_Room_Count(Request("ReservationID")) < 1 Then
    ...
    Protected Sub btnEmailRental_Click(sender As Object, e As System.EventArgs) Handles btnEmailRental.Click
        If Logic_Ensure_Reservation_Has_Room_And_Tour_Within_CheckIn_And_CheckOut() Or Login_Ensure_Hotel_Reservation_Has_No_Room_And_Tour_Within_CheckIn_And_CheckOut() Or CheckSecurity("Owners", "AlwaysShowLetter",,, Session("UserDBID")) Then
    ...
    Protected Sub btnPrintRental_Click(sender As Object, e As System.EventArgs) Handles btnPrintRental.Click
        ...
        If Login_Ensure_Hotel_Reservation_Has_No_Room_And_Tour_Within_CheckIn_And_CheckOut() Or CheckSecurity("Owners", "AlwaysShowLetter",,, Session("UserDBID")) Then
            Dim letterID As Integer = 0
            Dim oLetter As New clsReservationLetters
            letterID = oLetter.Find_Letter(txtReservationID.Text)
            If letterID > 0 Then
        ...
        ElseIf Logic_Ensure_Reservation_Has_Room_And_Tour_Within_CheckIn_And_CheckOut() Or CheckSecurity("Owners", "AlwaysShowLetter",,, Session("UserDBID")) Then
            Dim letterID As Integer = 0
            Dim oLetter As New clsReservationLetters
            letterID = oLetter.Find_Letter(txtReservationID.Text)
            If letterID > 0 Then
        ...
        End If
    End Sub
    Protected Sub lbSaveRes_Click(sender As Object, e As EventArgs) Handles lbSaveRes.Click
        Save_Res()
    End Sub
    Protected Sub Premiums_Link_Click(sender As Object, e As EventArgs) Handles Premiums_Link.Click
        If MultiView1.ActiveViewIndex <> 12 Then
            MultiView1.ActiveViewIndex = 13
            Premiums.KeyField = "ReservationID"
            Premiums.KeyValue = txtReservationID.Text
            Premiums.Display()
        End If
    End Sub
    Protected Sub Golf_Link_Click(sender As Object, e As EventArgs) Handles Golf_Link.Click
        Dim oRes2Golf As New clsRes2Golf
        gvGolf.DataSource = oRes2Golf.Get_Tee_Times(txtReservationID.Text)
        Dim sKeys(0) As String
        sKeys(0) = "ID"
    ...
    End Sub
    Protected Sub btnAddGolf_Click(sender As Object, e As EventArgs) Handles btnAddGolf.Click
        ClientScript.RegisterClientScriptBlock(Me.GetType, "AjaxCall", "modal.mwindow.open('EditTeeTime.aspx?ID=0&ReservationID=" & txtReservationID.Text & "','win01',350,350);", True)
    End Sub
    Protected Sub gvGolf_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvGolf.SelectedIndexChanged
        ...
        ClientScript.RegisterClientScriptBlock(Me.GetType, "AjaxCall", "modal.mwindow.open('EditTeeTime.aspx?ID=" & row.Cells(1).Text & "','win01',350,350);", True)
    End Sub
    Protected Sub gvGolf_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs)
        If e.Row.Cells(0).Text <> "No Tee Times" Then
            e.Row.Cells(1).Visible = False
            If e.Row.RowIndex > -1 Then
                e.Row.Cells(3).Text = CDate(e.Row.Cells(3).Text).ToShortDateString
                If e.Row.Cells(4).Text >= 1300 Then
                    e.Row.Cells(4).Text = Left(e.Row.Cells(4).Text, 2) - 12 & ":" & Right(e.Row.Cells(4).Text, 2) & " PM"
                ElseIf e.Row.Cells(4).Text >= 1200 Then
                    e.Row.Cells(4).Text = Left(e.Row.Cells(4).Text, 2) & ":" & Right(e.Row.Cells(4).Text, 2) & " PM"
                ElseIf e.Row.Cells(4).Text < 100 Then
                    If e.Row.Cells(4).Text < 10 Then
                        e.Row.Cells(4).Text = "12:0" & e.Row.Cells(4).Text & " AM"
                    Else
                        e.Row.Cells(4).Text = "12:" & e.Row.Cells(4).Text & " AM"
                    End If
                ElseIf e.Row.Cells(4).Text < 1000 Then
                    e.Row.Cells(4).Text = Left(e.Row.Cells(4).Text, 1) & ":" & Right(e.Row.Cells(4).Text, 2) & " AM"
                Else
                    e.Row.Cells(4).Text = Left(e.Row.Cells(4).Text, 2) & ":" & Right(e.Row.Cells(4).Text, 2) & " AM"
                End If
            End If
        End If
    End Sub

    Private Sub Logic_Print_Or_Email_Owner_Ltr()
        ...
        With New clsReservations
            ...
            .Load()
            is_kcp = New clsComboItems().Lookup_ComboItem(.ResLocationID).Equals("KCP")
        End With

        If String.IsNullOrEmpty(siStatus.SelectedName) = False And String.IsNullOrEmpty(siResType.SelectedName) = False _
            And String.IsNullOrEmpty(txtCheckInDate.Text) = False And is_kcp Then
            If (siStatus.SelectedName.Equals("Booked") And siResType.SelectedName.Equals("Owner") _
                And DateTime.Compare(DateTime.Parse(txtCheckInDate.Text), DateTime.Now) >= 0) Or CheckSecurity("Owners", "AlwaysShowLetter",,, Session("UserDBID")) Then
        ...
        End If
    End Sub
    Private Sub Logic_Print_Or_Email_Rental_Ltr()
        If String.IsNullOrEmpty(siStatus.SelectedName) = False And String.IsNullOrEmpty(siResType.SelectedName) = False _
           And String.IsNullOrEmpty(txtCheckInDate.Text) = False Then

            Dim reservation_id = Request("ReservationID")
            Dim res_location = String.Empty
            ...
            With New clsReservations
                .ReservationID = reservation_id
                .Load()
                res_location = New clsComboItems().Lookup_ComboItem(.ResLocationID)
                        ...
                        If (New clsTour).Get_Tour_By_Res(reservation_id) > 0 And .CheckInDate.Length > 0 And .CheckOutDate.Length > 0 Then
                            ...
                            If tour.get_Tour_Status(tour.TourID).Equals("Booked") Then
                                If String.IsNullOrEmpty(tour.TourDate) = False Then
                                    ...
                                    If DateTime.Compare(DateTime.Parse(tour.TourDate), DateTime.Parse(.CheckInDate)) > 0 And
                                         DateTime.Compare(DateTime.Parse(tour.TourDate), DateTime.Parse(.CheckOutDate)) < 0 Then
                                        tf = True
                                    End If
                                End If
                            End If
            ...
            If ((siStatus.SelectedName.Equals("Booked") Or siStatus.SelectedName.Equals("Pending Payment")) And (siResType.SelectedName.Equals("Marketing")) _
                And DateTime.Compare(DateTime.Parse(txtCheckInDate.Text), DateTime.Now) >= 0 And tf = True) Then

                btnEmailRental.Visible = True
                btnPrintRental.Visible = True
            End If

            If siResType.SelectedName = "(empty)" And siResSubType.SelectedName = "(empty)" And res_location = "Williamsburg" And tf = True Then
                btnEmailRental.Visible = True
                btnPrintRental.Visible = True
            End If
        End If
    ...
    Private Function Logic_Ensure_Reservation_Has_Room_And_Tour_Within_CheckIn_And_CheckOut() As Boolean
        ...
        With New clsReservations
            ...
            Dim is_kcp = New clsComboItems().Lookup_ComboItem(.ResLocationID).Equals("KCP")

            '//- check room availability
            If .Get_Room_Count(reservation_id) > 0 And is_kcp Then
        ...
        Return tf
    End Function
    Private Function Login_Ensure_Hotel_Reservation_Has_No_Room_And_Tour_Within_CheckIn_And_CheckOut() As Boolean
        Dim tf = False
        With New clsReservations
            .ReservationID = Request("ReservationID")
            .Load()

            Dim is_williamsburg = New clsComboItems().Lookup_ComboItem(.ResLocationID).Equals("Williamsburg")

            '//- must have no room since it is a hotel reservation
            If .Get_Room_Count(.ReservationID) = 0 And is_williamsburg Then
                '//- if reservation has a tour
                If (New clsTour).Get_Tour_By_Res(.ReservationID) > 0 Then
                    '\\- ensure tour is booked and falls with checkin and checkout
                    Dim tour = New clsTour
                    tour.TourID = (New clsTour).Get_Tour_By_Res(.ReservationID)
                    tour.Load()
                    If tour.get_Tour_Status(tour.TourID).Equals("Booked") Then
                        If String.IsNullOrEmpty(tour.TourDate) = False Then
