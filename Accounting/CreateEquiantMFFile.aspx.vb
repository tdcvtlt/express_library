    ...
    Private Function Get_SQL(ByVal index As Integer) As String
        ...
        Select Case index
            Case 1
                ...
                ret = "select  case when i.saletype = 'Estates' then '131100' + CAST(c.contractid as varchar(10)) when i.SaleType='Townes' then '133100' + CAST(c.contractid as varchar(10)) else '132100' + CAST(c.contractid as varchar(10)) end as Account, " &
                        "   'CHG' as Type, 'MF' as Category, inv.Amount, 'CHG-20' + RIGHT(inv.invoice,2) as 'TransactionCode', inv.TransDate as 'TransactionDate', inv.DueDate, Case when invoice = 'MF16' then DATEADD(dd,2,TransDate) when invoice = 'MF17' then '11/15/2017' when invoice = 'MF19' then '11/15/2018' else DATEADD(dd,1,getdate()) end as 'StatementDate' " &
                        "   From v_Invoices inv  " &
                        "	inner join t_Contract c on inv.keyvalue = c.ContractID " &
                        "	inner join v_ContractInventory i on i.ContractID = c.ContractID " &
                        "Where inv.keyfield = 'ContractID' and inv.invoice like 'MF%' and inv.invoice in (" & trans & ") and inv.TransDate between '" & dfStart.Selected_Date & "' and '" & dfEnd.Selected_Date & "'"
            Case Else
