...
Partial Class Accounting_CreateAFCFile
    ...
    Private Function Get_SQL(ByVal index As Integer) As String
        ...
        Select Case index
            Case 1
                ret = "select distinct p.prospectid as [Owner ID], c.ContractID, " & _
                        "             '' as [Owner Number]," & _
                        "             p.Firstname, " & _
                        "             p.Lastname, " & _
                    ...
                    " where c.ContractID in (select  ContractID from (" & _
                                ...
                                " 	(select sum(case when balance is null then 0 else balance end) from v_Invoices where keyfield = 'contractid' and keyvalue = c.contractid and invoice = '" & ddMF.SelectedItem.Text & "' AND TransDate BETWEEN '" & dfStart.Selected_Date & "' AND Cast('" & dfEnd.Selected_Date & "' as datetime) + 1) as [Current Balance Due] " & _
                                " from t_Contract c " & _
                                ...
                                " 	inner join (select * from v_Invoices where keyfield = 'contractid' and invoice = '" & ddMF.SelectedItem.Text & "' AND TransDate BETWEEN '" & dfStart.Selected_Date & "' AND Cast('" & dfEnd.Selected_Date & "' as datetime) + 1) i on i.keyvalue = c.contractid " & _
            ...
            Case 2
                ret = "select distinct  * from ( " & _
                        ...
                        " 	CASE WHEN (select sum(case when balance is null then 0 else balance end) from v_Invoices where keyfield = 'contractid' and keyvalue = c.contractid and invoice = '" & ddMF.SelectedItem.Text & "' AND TransDate BETWEEN '" & dfStart.Selected_Date & "' AND Cast('" & dfEnd.Selected_Date & "' as datetime) + 1) IS NULL THEN " & _
                        " 0 " & _
                        " 	ELSE " & _
                        " 		(select sum(case when balance is null then 0 else balance end) from v_Invoices where keyfield = 'contractid' and keyvalue = c.contractid and invoice = '" & ddMF.SelectedItem.Text & "' AND TransDate BETWEEN '" & dfStart.Selected_Date & "' AND Cast('" & dfEnd.Selected_Date & "' as datetime) + 1) " & _
