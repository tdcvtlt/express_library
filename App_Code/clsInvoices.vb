...
Public Class clsInvoices
    ...
    Public Function List_Invoices_With_Balance() As SQLDataSource
        ...
        Dim sql = "select a.*, case when (invoice in ('late fee', 'adminfee', 'banking fee', 'interest', 'legalfee', 'Return Check') and mf.ComboItem = 'AFC') or (left(invoice, 2) = 'mf' and mf.ComboItem = 'AFC') then 'AFC' when (invoice in ('late fee', 'adminfee', 'banking fee', 'interest', 'legalfee', 'Return Check') and mf.ComboItem = 'AFC - In Collections') or (left(invoice, 2) = 'mf' and mf.ComboItem = 'AFC - In Collections') then 'AFC - In Collections' end AFC from ufn_financials(" & _ProspectID & ", '" & _KeyField & "'," & _KeyValue & ",0)  a left join t_contract c on c.contractid = a.keyvalue left outer join t_COmboItems mf on c.MaintenanceFeeStatusID = mf.ComboItemID where balance > 0 order by transdate asc"
        ds.SelectCommand = Sql
        Return ds
    End Function

    Public Function List_Invoices_With_Balance_ByAcct(ByVal AcctID As Integer) As DataTable
        ...
        Try
            If cn.State <> ConnectionState.Open Then cn.Open()
            cm.CommandText = "select a.ID, a.Balance from ufn_financials(" & _ProspectID & ", '" & _KeyField & "'," & _KeyValue & ",0)  a left join t_contract c on c.contractid = a.keyvalue left outer join t_COmboItems mf on c.MaintenanceFeeStatusID = mf.ComboItemID where balance > 0 and Acct = " & AcctID & " order by transdate asc"
            dread = cm.ExecuteReader
            If dread.HasRows Then
                Do While dread.Read()
                    drow = dt.NewRow
                    drow("ID") = dread("ID")
                    drow("Balance") = dread("Balance")
                    dt.Rows.Add(drow)
                Loop
            End If
            dread.Close()
    ...
    Public Function List_Invoices_Oldest_First() As SqlDataSource
        Dim ds As New SqlDataSource
        ds.ConnectionString = Resources.Resource.cns
        ds.SelectCommand = "select a.*, case when (invoice in ('late fee', 'adminfee', 'banking fee', 'interest', 'legalfee', 'Return Check') and mf.ComboItem = 'AFC') or (left(invoice, 2) = 'mf' and mf.ComboItem = 'AFC') then 'AFC' when (invoice in ('late fee', 'adminfee', 'banking fee', 'interest', 'legalfee', 'Return Check') and mf.ComboItem = 'AFC - In Collections') or (left(invoice, 2) = 'mf' and mf.ComboItem = 'AFC - In Collections') then 'AFC - In Collections' end AFC from ufn_financials(" & _ProspectID & ", '" & _KeyField & "'," & _KeyValue & ",0) a left join t_contract c on c.contractid = a.keyvalue left outer join t_COmboItems mf on c.MaintenanceFeeStatusID = mf.ComboItemID order by a.transdate asc"
        Return ds
    End Function
    ...
    Public Function Find_Existing_Invoice(ByVal KeyField As String, ByVal KeyValue As Integer, ByVal TransCode As String, Optional balanceGT0 As Boolean = False) As Integer
        Dim iRet As Integer = 0
        cn.Open()
        If Not (balanceGT0) Then
        ...
        Else
            cm.CommandText = "Select *, id as InvoiceID from v_Invoices i where i.invoice = '" & TransCode & "' and i.keyfield = '" & KeyField & "' and i.keyvalue='" & KeyValue & "' and i.balance > 0"
        End If
        dread = cm.ExecuteReader
        If dread.HasRows Then
            dread.Read()
