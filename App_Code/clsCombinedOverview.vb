...
Public Class clsCombinedOverview
    ...
    Dim _LineIntervalsSales As Decimal = 0
    Dim _LineIntervalsCan As Decimal = 0
    ...
    Public Sub New()
        ...
        cm = New SqlCommand("Select * from t_CombinedOverview where CombinedOverviewID = " & _ID, cn)
    End Sub

    Public Sub Load()
        Try
            cm.CommandText = "Select * from t_CombinedOverview where CombinedOverviewID = " & _ID
            da = New SqlDataAdapter(cm)
            ds = New DataSet
            da.Fill(ds, "t_CombinedOverview")
            If ds.Tables("t_CombinedOverview").Rows.Count > 0 Then
                dr = ds.Tables("t_CombinedOverview").Rows(0)
                Set_Values()
            End If
    ...
    Private Sub Set_Values()
        ...
        If Not (dr("LineIntervalsSales") Is System.DBNull.Value) Then _LineIntervalsSales = dr("LineIntervalsSales")
        If Not (dr("LineIntervalsCan") Is System.DBNull.Value) Then _LineIntervalsCan = dr("LineIntervalsCan")
    ...
    Public Function Save() As Boolean
        Try
            ...
            If ds.Tables("t_CombinedOverview").Rows.Count > 0 Then
                dr = ds.Tables("t_CombinedOverview").Rows(0)
            Else
                da.InsertCommand = New SqlCommand("dbo.sp_CombinedOverviewInsert", cn)
                ...
                da.InsertCommand.Parameters.Add("@LineIntervalsSales", SqlDbType.Money, 0, "LineIntervalsSales")
                da.InsertCommand.Parameters.Add("@LineIntervalsCan", SqlDbType.Money, 0, "LineIntervalsCan")
                ...
                da.InsertCommand.Parameters.Add("@LineClosingProj", SqlDbType.Float, 0, "LineClosingProj")
                ...
                da.InsertCommand.Parameters.Add("@IHSalesProj", SqlDbType.Float, 0, "IHSalesProj")
                ...
                dr = ds.Tables("t_CombinedOverview").NewRow
            End If
            ...
            Update_Field("LineIntervalsSales", _LineIntervalsSales, dr)
            Update_Field("LineIntervalsCan", _LineIntervalsCan, dr)
            ...
            If ds.Tables("t_CombinedOverview").Rows.Count < 1 Then ds.Tables("t_CombinedOverview").Rows.Add(dr)
            da.Update(ds, "t_CombinedOverview")
            _ID = ds.Tables("t_CombinedOverview").Rows(0).Item("CombinedOverviewID")
    ...
    Public Function List(ByVal sDate As String, ByVal loc As String) As SQLDataSource
        ...
        Try
            ...
            If sDate = "" Then
                ds.SelectCommand = "Select Top 50 CombinedOverViewID as ID, OverviewDate as OverView from t_CombinedOverview where overviewlocation = '" & loc & "' order by combinedoverviewid desc"
            Else
                ds.SelectCommand = "Select CombinedOverViewID as ID, OverviewDate as OverView from t_CombinedOverview where overviewdate = '" & sDate & "' and overviewlocation = '" & loc & "'"
            End If
        Catch ex As Exception
            _Err = ex.Message
        End Try
    ...
    Public Function Check_Exists(ByVal sDate As String, ByVal loc As String) As Boolean
        Dim bValid As Boolean = True
        Try
            If cn.State <> ConnectionState.Open Then cn.Open()
            cm.CommandText = "Select * from t_CombinedOverview where overviewdate = '" & sDate & "' and overviewlocation = '" & loc & "'"
    ...
    Public Function Get_MTD_Total(ByVal sDate As String, ByVal loc As String, ByVal fieldName As String) As Double
        ...
        Try
            If cn.State <> ConnectionState.Open Then cn.Open()
            cm.CommandText = "Select Case when Sum(" & fieldName & ") is null then 0 else sum(" & fieldName & ") end as MTDTotal from t_CombinedOverView where overviewlocation = '" & loc & "' and Month(OverViewDate) = Month('" & CDate(sDate) & "') and Year(OverViewDate) = Year('" & CDate(sDate) & "') and OverViewDate <= '" & CDate(sDate) & "'"
            dread = cm.ExecuteReader
            dread.Read()
    ...
    Public Function Get_YTD_Total(ByVal sDate As String, ByVal loc As String, ByVal fieldName As String) As Double
        ...
        Try
            If cn.State <> ConnectionState.Open Then cn.Open()
            cm.CommandText = "Select Case when Sum(" & fieldName & ") is null then 0 else sum(" & fieldName & ") end as YTDTotal from t_CombinedOverView where overviewlocation = '" & loc & "' and Year(OverViewDate) = Year('" & CDate(sDate) & "') and OverViewDate <= '" & CDate(sDate) & "'"
            dread = cm.ExecuteReader
            dread.Read()
    ...
    Public Property LineIntervalsSales() As Decimal
        Get
            Return _LineIntervalsSales
        End Get
        Set(ByVal value As Decimal)
            _LineIntervalsSales = value
        End Set
    End Property

    Public Property LineIntervalsCan() As Decimal
        Get
            Return _LineIntervalsCan
        End Get
        Set(ByVal value As Decimal)
            _LineIntervalsCan = value
        End Set
    End Property
    ...
    Public Property IHSalesProj() As Decimal
        Get
            Return _IHSalesProj
        End Get
        Set(ByVal value As Decimal)
