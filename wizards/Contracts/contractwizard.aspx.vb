...
Partial Class wizards_Contracts_contractwizard
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not (IsPostBack) Then
            ...
            Session("Spouse") = Nothing
            Session("CoOwn_Table") = Nothing
            ...
            Session("Inventory_Table") = Nothing
            Session("Commissions") = Nothing
            Session("Contract") = Nothing
            Session("Mortgage") = Nothing
            Session("CoOwners") = Nothing
            Session("DPInvoice") = Nothing
            Session("CCInvoice") = Nothing
            Session("MemInvoice") = Nothing
    ...
    Protected Sub Load_Items()
        ...
        ds.SelectCommand = "Select ci.ComboItemID, ci.ComboItem from t_ComboItems ci inner join t_Combos c on ci.COmboID = c.ComboID where c.COmboName = 'ContractSubStatus' and ci.COmboItem in ('Cash','Funded','Developer','Pender') order by ci.Comboitem asc"
        ddSubStatus.DataSource = ds
        ddSubStatus.DataTextField = "ComboItem"
        ddSubStatus.DataValueField = "ComboItemID"
        ddSubStatus.DataBind()
        ddUGSeason.Items.Add(New ListItem("Red", "Red"))
        ddUGSeason.Items.Add(New ListItem("Yellow", "Yellow"))
        ddUGBD.Items.Add(New ListItem("1", "1"))
        ddUGBD.Items.Add(New ListItem("2", "2"))
        ddUGBD.Items.Add(New ListItem("3", "3"))
        ddUGBD.Items.Add(New ListItem("4", "4"))
        ddUGUT.Items.Add(New ListItem("Cottage", "Cottage"))
        ddUGUT.Items.Add(New ListItem("Townes", "Townes"))
        ddUGUT.Items.Add(New ListItem("Estates", "Esates"))
    ...
    Protected Sub btnNext_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNext.Click
        If Validate_Step(hfStep.Value) Then
            If hfStep.Value = 2 Then
                ...
                If oDNS.Get_Status(oTour.ProspectID) = "Remove" Then
                    ClientScript.RegisterClientScriptBlock(Me.GetType, "pop", "modal.mwindow.open('../../marketing/DoNotSellListOverRide.aspx?KeyField=Contract&KeyValue=" & oTour.ProspectID & "&Source=ContractWizard','win01',350,350);", True)
                Else
                    Increment_Step()
                End If
                oDNS = Nothing
        ...
        Else
            If sStepMessage = "DOCS" Then
                ClientScript.RegisterClientScriptBlock(Me.GetType, "none", "javascript:modal.mwindow.open('../../general/printcontracttest.aspx?contractid=" & oContract.ContractID & "','win01',1050,1050);", True)
            Else
    ...
    Private Function Validate_Step(ByVal iStep As Integer) As Boolean
        Select Case iStep
            Case 1
                ...
                Return (rbUD.Checked Or rbRe.Checked Or rbNew.Checked Or rbCon.Checked)
            Case 2
                ...
                'Else
                If rbNewContract.Checked Then
                    sStepMessage = "Please select / enter a valid Tour ID"
                    Return Validate_TourID()

                ElseIf rbUpgrade.Checked Then
                    sStepMessage = "Please Enter a Valid Contract Number"
                    ...
                    Else
                        Return False
                    End If
                ElseIf rbDownGrade.Checked Then
                    sStepMessage = "Please Enter a Valid Contract Number"
                    If Validate_Contract() Then
                        sStepMessage = "Please select / enter a valid Tour ID"
                        Return Validate_TourID()
                ...
                ElseIf rbCon.Checked Then
                    sStepMessage = "Please Enter a valid Contract Number"
                    Return False

                ElseIf rbRe.Checked Then
                    sStepMessage = "Please Enter a valid Current Contract Number"
                    sStepMessage = "Please Enter a valid Owner ID"
                    Return False
    ...
    Private Sub Set_View(ByVal iStep As Integer)
        ...
        'Response.Write(oPros.Prospect_ID)
        'Response.Write(String.Format("<strong>{0}</strong>", iStep.ToString()))
        ...
        ElseIf iStep = 4 Then
            ...
            'get contract number if txtcn is blank
            If txtContractNumber.Text = "" Then
                txtContractNumber.Text = oContract.Get_Next_Number()
            End If
        ElseIf iStep = 5 Then
            Save()
    ...
    Private Sub Set_View_6()
        ...
        Dim ny As Boolean = False
        For i = 0 To dtAddress.Rows.Count - 1
            If (New clsComboItems).Lookup_ComboItem(dtAddress.Rows(i).Item("StateID")) = "NY" And dtAddress.Rows(i).Item("Active") = "True" Then ny = True
        Next
        If oDPInvoice.InvoiceID = 0 Then
            ...
            If ny Then
                oDPInvoice.FinTransID = oFinTrans.Find_Fin_Trans("MortgageDP", "Down Payment Sale - NY")
            Else
        ...
        If oCCInvoice.InvoiceID = 0 Then
            ...
            If ny Then
                oCCInvoice.FinTransID = oFinTrans.Find_Fin_Trans("MortgageCC", "Closing Costs Sale - NY")
            Else
                If InStr(oCombo.Lookup_ComboItem(oContract.BillingCodeID), "Resort Finance") > 0 Then
    ...
    Private Sub Save()
        If rbNew.Checked Then 'Only TourID
            ...
            Save_Addresses(hfProspectID.Value)
            Save_Emails(hfProspectID.Value)
            Save_Phones(hfProspectID.Value)
            ...
            oUField.ID = oUField.Get_UserField_Value_ID(oUField.Get_UserFieldID(oUField.Get_GroupID("Contract"), "UpgradeOptionSeason"), oContract.ContractID)
            oUField.Load()
            If cbUpgradeOption.Checked Then
                oUField.UFValue = ddUGSeason.SelectedValue
            Else
                oUField.UFValue = ""
            End If
            oUField.UFID = oUField.Get_UserFieldID(oUField.Get_GroupID("Contract"), "UpgradeOptionSeason")
            oUField.KeyValue = oContract.ContractID
            oUField.Save()
            oUField.ID = oUField.Get_UserField_Value_ID(oUField.Get_UserFieldID(oUField.Get_GroupID("Contract"), "UpgradeOptionBD"), oContract.ContractID)
            oUField.Load()
            If cbUpgradeOption.Checked Then
                oUField.UFValue = ddUGBD.SelectedValue
            Else
                oUField.UFValue = ""
            End If
            oUField.UFID = oUField.Get_UserFieldID(oUField.Get_GroupID("Contract"), "UpgradeOptionBD")
            oUField.KeyValue = oContract.ContractID
            oUField.Save()
            oUField.ID = oUField.Get_UserField_Value_ID(oUField.Get_UserFieldID(oUField.Get_GroupID("Contract"), "UpgradeOptionUnitType"), oContract.ContractID)
            oUField.Load()
            If cbUpgradeOption.Checked Then
                oUField.UFValue = ddUGUT.SelectedValue
            Else
                oUField.UFValue = ""
            End If
            oUField.UFID = oUField.Get_UserFieldID(oUField.Get_GroupID("Contract"), "UpgradeOptionUnitType")
            oUField.KeyValue = oContract.ContractID
            oUField.Save()
            oUField.ID = oUField.Get_UserField_Value_ID(oUField.Get_UserFieldID(oUField.Get_GroupID("Contract"), "UpgradeOptionPrice"), oContract.ContractID)
            oUField.Load()
            If cbUpgradeOption.Checked Then
                oUField.UFValue = txtUGPrice.Text
            Else
                oUField.UFValue = ""
            End If
            oUField.UFID = oUField.Get_UserFieldID(oUField.Get_GroupID("Contract"), "UpgradeOptionPrice")
            oUField.KeyValue = oContract.ContractID
            oUField.Save()
            oUField = Nothing

            Dim oUPDown As New clsContractUpDownGrade
            oUPDown.ID = oUPDown.GetID(oContract.ContractID)
            oUPDown.Load()
            If rbNewContract.Checked And oUPDown.ID > 0 Then
                oUPDown.Delete_ID(oUPDown.ID)
            Else
                Dim oConOld As New clsContract
                oConOld.ContractNumber = txtOldContract.Text
                oConOld.Load()
                oUPDown.OldContractID = oConOld.ContractID
                oUPDown.NewContractID = oContract.ContractID
                If rbDownGrade.Checked Then
                    oUPDown.SaleType = "Downgrade"
                Else
                    oUPDown.SaleType = "Upgrade"
                End If
                oUPDown.Save()
                oConOld = Nothing
            End If
        ...
        ElseIf rbCon.Checked Then 'Current KCP

        ElseIf rbRe.Checked Then 'Old KCP
    ...
    Private Sub Save_Inventory(ByVal ContractID As Integer)
        ...
        oC.ContractID = ContractID
        oC.UserID = Session("UserDBID")
        oC.Load()
        oC.MaintenanceFeeCodeID = oC.Get_MF_Code(ContractID)
        oC.Save()
        oC = Nothing
    End Sub
    ...
    Private Sub Save_Contract(ByVal ParentID As Integer)
        ...
        'Update contract number to match sale type
        If InStr(siSalesType.SelectedName.toUpper, "DWN") > 0 Then
            If Not (IsNumeric(Left(txtContractNumber.Text, 1))) Then
                txtContractNumber.Text = Replace(txtContractNumber.Text, "A", "F")
            Else
                txtContractNumber.Text = "F" & txtContractNumber.Text
            End If
        ElseIf InStr(siSalesType.SelectedName.toUpper, "UPGRADE") > 0 Then
            If Not (IsNumeric(Left(txtContractNumber.Text, 1))) Then
                txtContractNumber.Text = Replace(txtContractNumber.Text, "F", "A")
            Else
                txtContractNumber.Text = "A" & txtContractNumber.Text
            End If
        Else
            If Not (IsNumeric(Left(txtContractNumber.Text, 1))) Then
                txtContractNumber.Text = Replace(txtContractNumber.Text, "F", "")
                txtContractNumber.Text = Replace(txtContractNumber.Text, "A", "")
            End If
        End If
        ...
        oContract.SubStatusID = ddSubStatus.SelectedValue
        ...
        oPros.Prospect_ID = ParentID
        oPros.Load()
        oPros.AnniversaryDate = IIf(oPros.AnniversaryDate = "", System.DateTime.Now, oPros.AnniversaryDate)
        oPros.Save()
        oPros = Nothing
        oContract.UserID = Session("UserDBID")
        'If Left(txtContractNumber.Text, 1) = "U" Then
        ...
        oCombo = Nothing
        'End If
        oContract.MaintenanceFeeCodeID = 0
        oContract.Save()
        If oContract.Error_Message <> "" Then Response.Write(oContract.Error_Message)
    ...
    Private Sub Load_Create_Mortgage()
        ...
        If oCI.Lookup_ComboItem(oContract.SubStatusID) = "Cash" Then
            oMortgage.StatusID = oCI.Lookup_ID("MortgageStatus", "N/A")
        ElseIf oCI.Lookup_ComboItem(oContract.SubStatusID) = "Funded" Or oCI.Lookup_ComboItem(oContract.SubStatusID) = "Developer" Or oCI.Lookup_ComboItem(oContract.SubStatusID) = "Pender" Then
            oMortgage.StatusID = oCI.Lookup_ID("MortgageStatus", "Active")
        End If
        oMortgage.Save()
        hfMortgageID.Value = oMortgage.MortgageID
    ...
    Private Sub Save_Addresses(ByVal ParentID As Integer)
        ...
        For i = 0 To dtAddress.Rows.Count - 1
            ...
            End If
        Next
        oAddress.ProspectID = hfProspectID.Value
        dtAddress = oAddress.Get_Table()
        Session("Address_Table") = dtAddress
    End Sub
    ...
    Private Sub Save_Phones(ByVal ParentID As Integer)
        ...
        For i = 0 To dtPhone.Rows.Count - 1
            ...
            End If
        Next
        oPhone.ProspectID = hfProspectID.Value
        dtPhone = oPhone.Get_Table()
        Session("Phone_Table") = dtPhone
    End Sub
    ...
    Private Sub Save_Emails(ByVal ParentID As Integer)
        ...
        For i = 0 To dtEmail.Rows.Count - 1
            ...
            If dtEmail.Rows(i).Item("Dirty") = True Then
                ...
                'dtEmail.Rows(i).Item("Dirty") = "False"
            End If
        Next
        oEmail.ProspectID = hfProspectID.Value
        dtEmail = oEmail.Get_Table()
        Session("Email_Table") = dtEmail
    End Sub
    ...
    Private Function Validate_Contract() As Boolean
        If txtOldContract.Text <> "" Then
            Dim oCon As New clsContract
            oCon.ContractNumber = txtOldContract.Text
            oCon.Load()
            If oCon.ProspectID > 0 Then
                Return True
            Else
                Return False
            End If
    ...
    Private Sub Set_View_3()
        ...
        If rbNew.Checked Then
            ...
            hfProspectID.Value = oPros.Prospect_ID.ToString()
    ...
    Private Sub Load_Tables()
        ...
        If Session("Owner") Is Nothing Then
            ...
            oContract.ContractNumber = IIf(rbNew.Checked, "", IIf(rbUD.Checked, txtOldKCP.Text, IIf(rbCon.Checked, txtOldKCP.Text, IIf(rbRe.Checked, txtRedeedOld.Text, ""))))
            oContract.Load()
            oTour.TourID = hfTourID.Value
            oTour.Load()
            ...
            Session("Address_Table") = dtAddress
            Session("Phone_Table") = dtPhone
            Session("Inventory_Table") = dtInventory
            Session("Contract") = oContract
            Session("Commissions") = dtCommissions
            Session("Mortgage") = oMortgage
            Session("DPInvoice") = oDPInvoice
            Session("CCInvoice") = oCCInvoice
            Session("MemInvoice") = oMemInvoice
        End If
        oPros = Session("Owner")
        oSpouse = Session("Spouse")
        oContract = Session("Contract")
        oMortgage = Session("Mortgage")
        dtEmail = Session("Email_Table")
        dtAddress = Session("Address_Table")
        dtPhone = Session("Phone_Table")
        dtInventory = Session("Inventory_Table")
        dtCommissions = Session("Commissions")
        oDPInvoice = Session("DPInvoice")
        oCCInvoice = Session("CCInvoice")
        oMemInvoice = Session("MemInvoice")
    ...
    Private Sub Update()
        ...
        'If timed out, oMortgage object would be null
        If oMortgage Is Nothing Then Return
...
oDPInvoice.load()
	    oMortgage.DPTotal = oDPInvoice.Amount
            ...
            oMortgage.Save()
            If bCreate Then
                oMortgage.CCTotal = oMortgage.Create_CC_Defaults(oMortgage.MortgageID, oMortgage.SalesPrice, Math.Round((oMortgage.SalesPrice + oMortgage.CCFinanced) * 0.1, 2))
                oMortgage.CCFinanced = If(cbFinanceCC.Checked, oMortgage.CCTotal, 0)
		'oMortgage.DPTotal = Math.Round((oMortgage.SalesPrice + oMortgage.CCFinanced) * 0.1, 2)
		If oMortgage.DPTotal < Math.Round((oMortgage.SalesPrice + oMortgage.CCFinanced) * 0.1, 2) Then
	                oMortgage.DPTotal = Math.Round((oMortgage.SalesPrice + oMortgage.CCFinanced) * 0.1, 2)
                End If
                oMortgage.Save()
            Else
                If bUpdate Then
                    oMortgage.CCFinanced = If(cbFinanceCC.Checked, oMortgage.CCTotal, 0)
                    oMortgage.CCTotal = oMortgage.Update_CC_Defaults(oMortgage.SalesPrice, oMortgage.MortgageID, oMortgage.DPTotal)
                    If oMortgage.DPTotal < Math.Round((oMortgage.SalesPrice + oMortgage.CCFinanced) * 0.1, 2) Then
	                    oMortgage.DPTotal = Math.Round((oMortgage.SalesPrice + oMortgage.CCFinanced) * 0.1, 2)
                    End If
                    oMortgage.Save()
    ...
    Protected Sub OD_Updated() Handles dtOriginationDate.Date_Updated
        dtFirstPaymentDate.Selected_Date = CDate(DateAdd(DateInterval.Day, 65, CDate(dtOriginationDate.Selected_Date))).ToShortDateString
        'dtFirstPaymentDate.
        'Response.Write("Here")
    End Sub
    ...
    Protected Sub rbUD_CheckedChanged(sender As Object, e As System.EventArgs) Handles rbUD.CheckedChanged

    End Sub

    Protected Sub lbIncStep_Click(sender As Object, e As EventArgs) Handles lbIncStep.Click
        Increment_Step()
    End Sub
    Protected Sub rbUpgrade_CheckedChanged(sender As Object, e As EventArgs) Handles rbUpgrade.CheckedChanged
        lbOld.Visible = True
        txtOldContract.Visible = True
        txtOldContract.Text = ""
    End Sub
    Protected Sub rbDownGrade_CheckedChanged(sender As Object, e As EventArgs) Handles rbDownGrade.CheckedChanged
        lbOld.Visible = True
        txtOldContract.Visible = True
        txtOldContract.Text = ""
    End Sub
    Protected Sub rbNew_CheckedChanged(sender As Object, e As EventArgs) Handles rbNew.CheckedChanged


    End Sub
    Protected Sub rbNewContract_CheckedChanged(sender As Object, e As EventArgs) Handles rbNewContract.CheckedChanged
        lbOld.Visible = False
        txtOldContract.Visible = False
        txtOldContract.Text = ""
    End Sub
    Protected Sub cbUpgradeOption_CheckedChanged(sender As Object, e As EventArgs) Handles cbUpgradeOption.CheckedChanged
        If cbUpgradeOption.Checked Then
            lblSeason.Visible = True
            lblBD.Visible = True
            lblUT.Visible = True
            lblPrice.Visible = True
            ddUGSeason.Visible = True
            ddUGBD.Visible = True
            ddUGUT.Visible = True
            txtUGPrice.Visible = True
        Else
            lblSeason.Visible = False
            lblBD.Visible = False
            lblUT.Visible = False
            lblPrice.Visible = False
            ddUGSeason.Visible = False
            ddUGBD.Visible = False
            ddUGUT.Visible = False
            txtUGPrice.Visible = False
        End If
    End Sub
End Class
