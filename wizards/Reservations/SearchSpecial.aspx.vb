...
Partial Class wizard_Reservations_SearchSpecial
...
#Region "Event Handlers"
            ...
            If IsPostBack = False Then
                ...
                Select Case wiz.Scenario
                    Case EnumScenario.Ten
                        ...
                        ddStayDays.DataSource = Enumerable.Range(1, 2)
                        ddStayDays.DataBind()
        ...
        Protected Sub btSearch_Click(sender As Object, e As System.EventArgs) Handles btSearch.Click
            ...
            wiz.Search_CheckIn_Date = datePicker1.Text
            wiz.Search_CheckOut_date = wiz.Search_CheckIn_Date.Value.AddDays(ddStayDays.SelectedValue)
            ...
            Select Case wiz.Scenario
                Case EnumScenario.Ten
                    ...
                    Try
                        ...
                        IFind = New Vrc_Packages_Available(arr_p, arr_u)
                    Else
                        ...
                        With New clsVendor()
                            ...
                            If .Vendor.ToLower() = "Grand Incentives".ToLower() Then
                                IFind = New Grand_Incentives_PackagesAvailable(arr_p, arr_u, vendorID)

                            ElseIf .Vendor.ToLower() = "Complete Call Solutions".ToLower() Then
                                IFind = New Complete_Call_Solutions(arr_p, arr_u)
                            ElseIf .Vendor.ToLower() = "UP4ORCE".ToLower() Then
                                IFind = New Grand_Incentives_PackagesAvailable(arr_p, arr_u, vendorID)
                            Else
                                Return
                            End If
                    ...
                    Dim upf = New Grand_Incentives_PackagesAvailable(arr_p, arr_u, 72)
                    Dim abc = upf.Search(wiz.Search_CheckIn_Date, ddStayDays.SelectedItem.Text)
    ...
    Public Shared Function ufn_ToursAvailability(package_id As Int32, tour_date As DateTime, nights_stay As Int32) As String
        ...
        End With
        Dim dt = (New Package_Xtra).Get_Tours_Available(DateTime.Parse(tour_date), campaign_id, tour_location_id)
...
Namespace Special
    ...
    Class Vrc_Packages_Available
        ...
        End Function
    End Class
    Class Grand_Incentives_PackagesAvailable
        Implements IFind

        Private byPackageTypes As String()
        Private byUnitTypes As String()
        Private package_base As New Base_Package
        Private vendor_id As Int32

        Public Sub New(packageTypes As String(), unitTypes As String(), vendor_id As Int32)
            Me.byPackageTypes = packageTypes
            Me.byUnitTypes = unitTypes
        ...
        Public Function Search(checkin_date As Date, nights_stay As Int32) As DataTable Implements IFind.Search
            ...
            HttpContext.Current.Response.Write(vendor_id)
            Dim ds = New clsReservationWizard().Available_Packages(checkin_date.ToShortDateString(), nights_stay, 1, "crms.kingscreekplantation.com", vendor_id)
            Try
                Dim dt = ds.Tables(1).Rows.OfType(Of DataRow).Where(Function(x) x("PackageType").ToString() <> "Tradeshow" And x("PackageType").ToString() <> "Tour Package").CopyToDataTable()
                dt.Columns.Add("UnitType")
                For Each dr As DataRow In dt.Rows
