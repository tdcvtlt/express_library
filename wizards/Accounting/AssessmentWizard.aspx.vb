...
Partial Class wizards_Accounting_AssessmentWizard
    ...
    Dim sReport As String = "Invoices/Club Dues.rpt"
    ...
    Protected Sub rblType_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles rblType.SelectedIndexChanged
        ...
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2012 Maintenance Fee Invoice - Monthly.doc", "2012 Maintenance Fee Invoice - Monthly")
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2012 Late Fee Invoice.doc", "Late Fee Invoice")
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2013 Maintenance Fee Invoice - Monthly.doc", "Maintenance Fee Invoice - Monthly")

        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2013 Club Explorer Fee Invoice - Monthly.doc", "2013 Club Fee Invoice - Monthly")
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2013 Club Explorer Late Fee Invoice.doc", "2013 Late Club Fee Invoice")
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2014 Club Explorer Fee Invoice - Monthly.doc", "2014 Club Fee Invoice - Monthly")
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2014 Club Explorer Late Fee Invoice.doc", "2014 Late Club Fee Invoice")
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2015 Club Explorer Fee Invoice - Monthly.doc", "2015 Club Fee Invoice - Monthly")
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2015 Club Explorer Late Fee Invoice.doc", "2015 Late Club Fee Invoice")
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2016 Club Explorer Fee Invoice - Monthly.doc", "2016 Club Fee Invoice - Monthly")
        WordTemplates.Add("\\RS-FS-01\UploadedContracts\mis\2016 Club Explorer Late Fee Invoice.doc", "2016 Late Club Fee Invoice")
        ...
        ElseIf MultiView1.ActiveViewIndex = 3 Or MultiView1.ActiveViewIndex = 4 Then
            ...
            ddTemplate.Items.Add(New ListItem(WordTemplates.ElementAt(9).Value, WordTemplates.ElementAt(9).Key))
            ddTemplate.Items.Add(New ListItem(WordTemplates.ElementAt(10).Value, WordTemplates.ElementAt(10).Key))
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            BindData()
            Load_Dues()
            btnPrint.Text = "Print Last Assessment"
            btnPrintLF.Text = "Print Last Assessment"

        Else
            If Session("Report") Is Nothing Then
            Else
                CFViewer.ReportSource = Session("Report")
            End If
        End If
    End Sub
            ...
            If dr.HasRows Then
                While dr.Read
                    ...
                    If item.Text = "Active" Or item.Text = "On Hold" Or item.Text = "OnHold" Then
                        ...
                        lbCEStatuses.ClearSelection()
                        lbCEStatuses.Items.Add(item)
                    Else
                        lbChoices.ClearSelection()
                        lbChoices.Items.Add(item)
                        lbCESelect.ClearSelection()
                        lbCESelect.Items.Add(item)
                    End If
                End While
            End If
            dr.Close()
    ...
    Protected Sub btnPreview_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPreview.Click
        ...
        sSQL = "select * from (	 " & _
                        ...
                        "    left outer join t_Frequency f on f.frequencyid = c.frequencyid  " & _
                        "   left outer join ( " & _
                        "		select keyvalue as Contractid, sum(case when i.posneg = 0 then i.Amount else i.amount * -1 end + case when a.adj is null then 0 else a.adj end) as Total " & _
                        "		from t_Invoices i " & _
                        "           left outer join ( " & _
                        "               select a.applytoid, sum(case when posneg = 0 then amount else amount * -1 end) as Adj " & _
                        "               from t_Invoices a " & _
                        "               where a.applytoid > 0 " & _
                        "               Group by a.applytoid " & _
                        "           ) a on a.applytoid = i.invoiceid " & _
                        "           inner join t_FintransCodes f on f.fintransid = i.fintransid " & _
                        "           inner join t_Comboitems tc on tc.comboitemid = f.transcodeid " & _
                        "		where i.keyfield = 'contractid' and i.applytoid =0  " & _
                        "			and tc.comboitem = '" & ddYear.SelectedItem.Text & "' " & _
                        "		group by keyvalue) as i on i.contractid = c.contractid " & _
                        "   left outer join t_Comboitems cs on cs.comboitemid = c.statusid " & _
                        "   left outer join t_Comboitems css on css.comboitemid = c.substatusid " & _
                        "   left outer join t_Comboitems mfs on mfs.comboitemid = c.maintenancefeestatusid " & _
                        "where c.prospectid not in (15175110,15157867, 15187541) and c.statusid in (" & sStatus & ")  " & _
                        "	and (" & MFYear & "-year(c.occupancydate))%f.interval = 0  " & _
                        "	and year(c.occupancydate)<= " & MFYear & " " & _
                        "    and st.comboitem <> 'trial' " & _
                        "	and c.contractdate <='" & dfCuttOff.Selected_Date & "' " & _
    ...
    Private Sub CreateDocument(ByVal prospectId As String)
        ...
        If Not (InStr(ddTemplate.SelectedItem.Text, "Late") > 0) Then
            ...
            sql = "select Distinct(p.ProspectID), p.LastName, p.FirstName, (select top 1 address1 from t_Prospectaddress where prospectid = p.prospectid order by activeflag desc) as Address1, (select top 1 city from t_Prospectaddress where prospectid = p.prospectid order by activeflag desc) as City, (select top 1 comboitem from t_Prospectaddress a left outer join t_Comboitems s on s.comboitemid = a.stateid where a.prospectid = p.prospectid order by activeflag desc) as State, (select top 1 PostalCode from t_Prospectaddress where prospectid = p.prospectid order by activeflag desc) as PostalCode, coalesce((select top 1 co.comboitem from t_Prospectaddress pa inner join t_Comboitems co on co.comboitemid = pa.countryid where prospectid = p.prospectid order by activeflag desc), '' ) as Country, " & _
                    ...
                    "i.Balance + coalesce(v.BalanceForward, 0)  TotalDue, i.invoice_description " & _
                    "from " & _
                    "t_Prospect p left outer join " & _
                    "(select KeyValue,  Sum(Balance) BalanceForward from v_invoices " & _
                    "where (KeyField = 'ProspectID')  and Duedate <= '" & DueDate.Selected_Date & "' and (invoice <> '" & ClubFees.SelectedItem.Text & "') group by KeyValue) v " & _
            ...
            If (dr.HasRows = True) Then
                Do While dr.Read()
                    ...
                    Add_Item_Value("<TOTALDUE>", FormatCurrency(dr("TOTALDUE")), oDoc.PendingWordDocumentID)
                    'Add_Item_Value("<TRI>", " ", oDoc.PendingWordDocumentID)
                    'oDoc.PendingWordDocumentID = oDoc.PendingWordDocumentID
                    'oDoc.Load()
                    oDoc.Ready = False
                    oDoc.Save()
                    With oUD
                        ...
                        .Load()
                        .KeyField = "ProspectID"
                        .KeyValue = dr("ProspectID")
                        .ContentText = "Report:Club Dues.rpt::" & oDoc.PendingWordDocumentID
                        .DateUploaded = Date.Now
                        .Name = dr("INVOICE_DESCRIPTION") & ""
                        .Path = ""
                        .UploadedByID = Session("UserDBID")
        ...
        Else
            ...
            If (dr.HasRows = True) Then
                Do While dr.Read()
                    ...
                    If Not (dr("Invoice_Description") Is System.DBNull.Value) Then
                        Add_Item_Value("<DESCRIPTION>", dr("Invoice_Description") & "", oDoc.PendingWordDocumentID)
                    Else
                    ...
                    With oUD
                        ...
                        .KeyField = "ProspectID"
                        .KeyValue = dr("ProspectID")
                        .ContentText = "Report:Club Dues Late Fee.rpt:LATE FEE:" & oDoc.PendingWordDocumentID
                        .DateUploaded = Date.Now
                        .Name = dr("INVOICE_DESCRIPTION") & ""
                        .Path = ""
                        .UploadedByID = Session("UserDBID")
    ...
    Protected Sub btnLFPreview_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLFPreview.Click
        ...
        Dim sql As String = "select p.lastname as [Last Name], p.firstname as [First Name], c.contractnumber as [KCP#], i.Invoice as TransCode, i.amount as [AmountInvoiced], i.balance as Balance, " & txtLFAmount.Text & " as [LFAmount],  inv.saletype as Type, cs.comboitem as Status, css.comboitem as SubStatus, mfs.comboitem as MFStatus, c.ContractID " & _
                            ...
                            "	and  i.balance > 0 and  i.balance -100 >  " & _
                            ...
                            "   left outer join t_Comboitems css on css.comboitemid = c.substatusid " & _
                            "   left outer join t_Comboitems mfs on mfs.comboitemid = c.maintenancefeestatusid " & _
                            "where c.prospectid not in (15175110,15157867, 15187541) and c.statusid not in (select comboitemid from t_Comboitems i inner join t_Combos c on c.comboid = i.comboid where c.comboname='ContractStatus' and i.comboitem like 'C%') and i.invoice = 'MF' + cast(cast(right('" & ddLFYear.SelectedItem.Text & "', 2) as int) -2 as varchar)  " & _
                            "	and  i.balance > 0 and  i.balance -100 >  " & _
                            "		case when inv.frequency <> 'Triennial' then  " & _
                            "           0 " & _
                            "		else  " & _
...
" order by p.lastname, p.firstname"
        ...
        For Each oRow As GridViewRow In gvLF.Rows
            If Not (oRow.FindControl("cb") Is Nothing) Then
                If CType(oRow.FindControl("cb"), CheckBox).Checked Then
                    ...
                    invID = 0
                    If CDbl(oRow.Cells(5).Text) > 0 Then 'Find the existing Invoice
                        invID = oInv.Find_Existing_Invoice("ContractID", conid, oRow.Cells(4).Text) ' ddLFYear.SelectedItem.Text)
                    End If
                    oInv.Load()
    ...
    Protected Sub Preview_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Preview.Click
        ...
        Dim sStatus As String = ""
        For i = 0 To lbCEStatuses.Items.Count - 1
            sStatus &= IIf(sStatus = "", lbCEStatuses.Items(i).Value, "," & lbCEStatuses.Items(i).Value)
        Next
        ...
        '                     "where p.prospectid not in (15175110,15157867, 15187541) and year(a.an) < " & sYear & " " & _
        ...
        '                         "inner join t_Soldinventory s on s.contractid = c.contractid " & _
        '                         "inner join t_Comboitems st on st.comboitemid = c.subtypeid " & _
        '                        "where c.prospectid not in (15175110,15157867, 15187541) and cs.comboitem in ('Active', 'Suspense','Developer','ReDeed','In Bankruptcy','In Foreclosure','Reverter','InColl-Active', 'Deed-In-Lieu', 'InColl-Developer','Pending ReDeed') " & _
        '                         "and st.comboitem = 'Points' " & _
        '                       ") " & _
        '                    ") a"
        Dim sql As String = "select *,0 as [AmountInvoiced], case when Frequency = 'Annual' then 179 when Frequency = 'Biennial' then 132 else 117 end as [AssessmentAmount],case when Frequency = 'Annual' then 179 when Frequency = 'Biennial' then 132 else 117 end as [AmounttoAssess] " & _
                             ...
                             "select distinct p.ProspectID, p.Lastname as [LastName], p.firstname as [FirstName],  " & _
                              ...
                              "( " & _
                               ...
                               "from t_Contract c " & _
                                ...
                                "inner join t_Soldinventory s on s.contractid = c.contractid " & _
                                "inner join t_Frequency f on f.frequencyid = s.frequencyid " & _
                               "where cs.comboitemid in (" & sStatus & ") " & _
                                "and c.prospectid = p.prospectid " & _
                               "order by f.Frequency  " & _
                              ") as Frequency, " & _
                              "( " & _
                             ...
                             "where p.prospectid not in (15175110,15157867, 15187541) and year(a.an) < " & sYear & " " & _
                              ...
                              "and p.prospectid in ( " & _
                                ...
                                "from t_Contract c  " & _
                                 ...
                                 "inner join t_Soldinventory s on s.contractid = c.contractid " & _
                                 "inner join t_Comboitems st on st.comboitemid = c.subtypeid " & _
                                "where c.prospectid not in (15175110,15157867, 15187541) and cs.comboitemid in (" & sStatus & ") " & _
                                 "and st.comboitem = 'Points' " & _
                               ") " & _
                            ") a"
        'cdIndex = 183
    ...
    Protected Sub btnLFCFPreview_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLFCFPreview.Click
        ...
        Dim sql As String = "select p.ProspectID, p.lastname as [Last Name], p.firstname as [First Name], i.Invoice as TransCode, i.amount as [AmountInvoiced], i.balance as Balance, " & txtCDLFAMt.Text & " as [LFAmount] " & _
                            ...
                            "   where p.prospectid not in (15175110,15157867, 15187541) and i.invoice = '" & ddClubFee.SelectedItem.Text & "'  " & _
                            "	and i.balance > 0 " & _
                            "	and i.id not in ( " & _
        ...
        For Each oRow As GridViewRow In gvCDLF.Rows
            If Not (oRow.FindControl("cb") Is Nothing) Then
                If CType(oRow.FindControl("cb"), CheckBox).Checked Then
                    ...
                    invID = 0
                    If CDbl(oRow.Cells(5).Text) > 0 Then 'Find the existing Invoice
                        invID = oInv.Find_Existing_Invoice("ProspectID", oRow.Cells(1).Text, ddClubFee.SelectedItem.Text, True)
                    End If
                    oInv.Load()
    ...
    Protected Sub btnLFCFExport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLFCFExport.Click
        Response.Clear()
        Response.ContentType = "application/vnd.ms-excel"
        Response.AddHeader("Content-Disposition", "attachment; filename=" & IIf(rblType.SelectedValue = "CD", "Club Fee Assessment ", "Late Fee Assessment ") & "for " & ddClubFee.SelectedItem.Text & ".xls")
        Response.Write("<table>")
        Response.Write("<tr>")
    ...
    Protected Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        CFViewer.Visible = True
        Session("Report") = Nothing
        sReport = "Invoices/Club Dues.rpt"
        ...
        Report.SetParameterValue("@SID", "psb5mqvco31cilbzmk3t4uus") 'Session("SessionID"))
        Report.SetParameterValue("@ID", 0) 'Session("SessionID"))
        Session.Add("Report", Report)
        CFViewer.ReportSource = Session("Report")

    End Sub
    Protected Sub btnPrintLF_Click(sender As Object, e As EventArgs) Handles btnPrintLF.Click
        ' Response.Write(Session("SessionID"))
        CFViewer.Visible = True
        Session("Report") = Nothing
        sReport = "Invoices/Club Dues Late Fee.rpt"
        Report.Load(Server.MapPath(sReport))
        Report.FileName = Server.MapPath(sReport)
        'Response.Write("HERE")
        'Dim categoryID As Integer = Convert.ToInt32(ddlCategory.SelectedValue)
        Report.DataSourceConnections(0).SetConnection(Resources.Resource.SERVER, Resources.Resource.DATABASE, Resources.Resource.USERNAME, Resources.Resource.PASSWORD)
        'Report.DataSourceConnections(0).SetLogon(Resources.Resource.USERNAME, Resources.Resource.PASSWORD)
        Report.SetParameterValue("@SID", "LATE FEE")
        Report.SetParameterValue("@ID", 0) 'Session("SessionID"))
        Session.Add("Report", Report)
        CFViewer.ReportSource = Session("Report")
    ...
    Protected Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        If lbCESelect.SelectedIndex > -1 Then
            Dim item As ListItem = lbCESelect.SelectedItem
            lbCEStatuses.ClearSelection()
            lbCEStatuses.Items.Add(item)
            lbCESelect.Items.Remove(item)
        End If
    End Sub
    Protected Sub btnRemove_Click(sender As Object, e As EventArgs) Handles btnRemove.Click
        If lbCEStatuses.SelectedIndex > -1 Then
            Dim item As ListItem = lbCEStatuses.SelectedItem
            lbCESelect.ClearSelection()
            lbCESelect.Items.Add(item)
            lbCEStatuses.Items.Remove(item)
        End If
    End Sub
End Class
