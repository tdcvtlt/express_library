...
Partial Class frontdesk_SingleRegCard
    ...
    Private Sub Get_Reg_Card()
        ...
        rs.open("SELECT * FROM (SELECT DISTINCT(r.ReservationID), p.LastName + ',' + ' ' + p.FirstName as ProspectName, pa.Address1, pa.City, c.ComboItem as State, pa.PostalCode, r.CheckInDate, r.CheckOutDate, rest.ComboItem as ResType, resst.ComboItem as ResSubType " & _
              ...
              "WHERE rl.ComboItem = 'KCP' and (st.ComboItem = 'Booked' or st.ComboItem = 'In-House') AND (pa.prospectid is null or pa.activeflag = 1) and r.reservationid = '" & Request("Reservationid") & "') a " & _
        ...
        Else
            ...
            Do While Not rs.eof
                ...
                Lit.Text &= "<tr>"
                Lit.Text &= "<td>_________</td>"
                Lit.Text &= "<td><b><font size='2'>KING'S CREEK PLANTATION PROVIDES FREE ACCESS TO AN UNSECURED WIRELESS NETWORK IN EACH UNIT. THIS FREE SERVICE IS AN OPEN NETWORK PROVIDED FOR YOUR CONVENIENCE AND ITS USE IS AT YOUR OWN RISK. THE SERVICE IS PROVIDED ON AN AS AVAILABLE BASIS WITHOUT WARRANTIES OF ANY KIND. NO ADVICE OR INFORMATION GIVEN BY KING'S CREEK PLANTATION, PROVIDERS, AFFILIATES, OR CONTRACTORS OF THE SERVICE OR THEIR RESPECTIVE EMPLOYEES SHALL CREATE SUCH A WARRANTY.</font></b><br><br></td>"
                Lit.Text &= "</tr>"

                Lit.Text &= "<tr>"
                Lit.Text &= "<td>________</td>"
                Lit.Text &= "<td><b><font size='2'>IN THE EVENT I SECURE TEE TIMES THROUGH THE CONCIERGE FOR WILLIAMSBURG NATIONAL GOLF COURSE, I AM AWARE OF THE 24 HOUR CANCELLATION POLICY AND THAT I WILL BE CHARGED A NO SHOW FEE IF TEE TIME IS NOT CANCELLED WITHIN THE ALLOTTED TIME.</font></b></td>"
                Lit.Text &= "</tr>"
